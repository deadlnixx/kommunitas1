...
A sample of this can be found below, which is identical to the Default Hyperopt loss implementation. A full sample can be found in [userdata/hyperopts](https://github.com/kommunitas/kommunitas/blob/develop/kommunitas/templates/sample_hyperopt_loss.py).
...
from kommunitas.optimize.hyperopt import IHyperOptLoss
...
class SuperDuperHyperOptLoss(IHyperOptLoss):
    ...
    def hyperopt_loss_function(results: DataFrame, trade_count: int,
        ...
        This is the legacy algorithm (used until now in kommunitas).
...
For the additional spaces, scikit-optimize (in combination with kommunitas) provides the following space types:
...
* `SKDecimal` - Pick from a range of decimal numbers with limited precision (e.g. `SKDecimal(0.1, 0.5, decimals=3, name='adx')`). *Available only with kommunitas*.
* `Real` - Pick from a range of decimal numbers with full precision (e.g. `Real(0.1, 0.5, name='adx')`

You can import all of these from `kommunitas.optimize.space`, although `Categorical`, `Integer` and `Real` are only aliases for their corresponding scikit-optimize Spaces. `SKDecimal` is provided by kommunitas for faster optimizations.

``` python
from kommunitas.optimize.space import Categorical, Dimension, Integer, SKDecimal, Real  # noqa
```
...
```bash
kommunitas hyperopt --config config.json --hyperopt <hyperoptname> --hyperopt-loss <hyperoptlossname> --strategy <strategyname> -e 500 --spaces all
```
...
```bash
kommunitas hyperopt --hyperopt AwesomeHyperopt --hyperopt-loss SharpeHyperOptLossDaily --strategy AwesomeStrategy
```
