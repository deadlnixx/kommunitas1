...
```
usage: kommunitas backtesting [-h] [-v] [--logfile FILE] [-V] [-c PATH]
...
If no data is available for the exchange / pair / timeframe combination, backtesting will ask you to download them first using `kommunitas download-data`.
For details on downloading, please refer to the [Data Downloading](data-download.md) section in the documentation.

The result of backtesting will confirm if your bot has better odds of making a profit than a loss.

All profit calculations include fees, and kommunitas will use the exchange's default fees for the calculation.
...
!!! Note
    By default, kommunitas will export backtesting results to `user_data/backtest_results`.
    The exported trades can be used for [further analysis](#further-backtest-result-analysis) or can be used by the [plotting sub-command](plotting.md#plot-price-and-indicators) (`kommunitas plot-dataframe`) in the scripts directory.
...
```bash
kommunitas backtesting --strategy AwesomeStrategy
```
...
```bash
kommunitas backtesting --strategy AwesomeStrategy --timeframe 1m
```
...
```bash
kommunitas backtesting --strategy AwesomeStrategy --dry-run-wallet 1000
```
...
```bash
kommunitas backtesting --strategy AwesomeStrategy --datadir user_data/data/bittrex-20180101 
```
...
```bash
kommunitas backtesting --strategy-list SampleStrategy1 AwesomeStrategy --timeframe 5m
```
...
```bash
kommunitas backtesting --strategy backtesting --export none --config config.json 
```
...
```bash
kommunitas backtesting --strategy backtesting --export trades --export-filename=backtest_samplestrategy.json
```
...
```bash
kommunitas backtesting --fee 0.001
```
...
```bash
kommunitas backtesting --timerange=20190501-
```
...
``` bash
kommunitas backtesting --timerange 20180401-20180410 --timeframe 5m --strategy-list Strategy001 Strategy002 --export trades
```
