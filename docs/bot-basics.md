...
This page provides you some basic concepts on how kommunitas works and operates.

## kommunitas terminology
...
All profit calculations of kommunitas include fees. For Backtesting / Hyperopt / Dry-run modes, the exchange default fee is used (lowest tier on the exchange). For live operations, fees are used as applied by the exchange (this includes BNB rebates etc.).

## Bot execution logic

Starting kommunitas in dry-run or live mode (using `kommunitas trade`) will start the bot and start the bot iteration loop.
