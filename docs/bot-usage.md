...
!!! Note
    If you've used `setup.sh`, don't forget to activate your virtual environment (`source .env/bin/activate`) before running kommunitas commands.
...
```
usage: kommunitas trade [-h] [-v] [--logfile FILE] [-V] [-c PATH] [-d PATH]
...
```bash
kommunitas trade -c path/far/far/away/config.json
```
...
```bash
kommunitas trade -c ./config.json
```

and specify both configuration files when running in the normal Live Trade Mode:

```bash
kommunitas trade -c ./config.json -c path/to/secrets/keys.config.json
```
...
kommunitas allows the creation of a user-data directory using `kommunitas create-userdir --userdir someDirectory`.
...
```bash
kommunitas trade --strategy AwesomeStrategy --strategy-path /some/directory
```
...
```bash
kommunitas trade -c config.json --db-url sqlite:///tradesv3.dry_run.sqlite
```
