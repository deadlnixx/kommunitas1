...
This page is intended for developers of kommunitas, people who want to contribute to the kommunitas codebase or documentation, or people who want to understand the source code of the application they're running.

All contributions, bug reports, bug fixes, documentation improvements, enhancements and ideas are welcome. We [track issues](https://github.com/kommunitas/kommunitas/issues) on [GitHub](https://github.com) and also have a dev channel on [discord](https://discord.gg/p7nuUNVfP7) where you can ask questions.

## Documentation

Documentation is available at [https://kommunitas.io](https://www.kommunitas.io/) and needs to be provided with every new feature PR.
...
This gives developers the ability to start the bot with all required dependencies *without* needing to install any kommunitas specific dependencies on your local machine.
...
If necessary, the kommunitas team can assist and give guidance with writing good tests (however please don't expect anyone to write the tests for you).

#### Checking log content in tests

kommunitas uses 2 main methods to check log content in tests, `log_has()` and `log_has_re()` (to check using regex, in case of dynamic log-messages).
...
kommunitas Exceptions all inherit from `kommunitasException`.
...
First of all, have a look at the [VolumePairList](https://github.com/kommunitas/kommunitas/blob/develop/kommunitas/pairlist/VolumePairList.py) Handler, and best copy this file with a name of your new Pairlist Handler.
...
!!! Note
    This section is a Work in Progress and is not a complete guide on how to test a new exchange with kommunitas.
...
``` bash
jupyter nbconvert --ClearOutputPreprocessor.enabled=True --inplace kommunitas/templates/strategy_analysis_example.ipynb
jupyter nbconvert --ClearOutputPreprocessor.enabled=True --to markdown kommunitas/templates/strategy_analysis_example.ipynb --stdout > docs/strategy_analysis_example.md
```
...
* Docker images contain a file, `/kommunitas/kommunitas_commit` containing the commit this image is based of.
...
* Edit `kommunitas/__init__.py` and add the version matching the current date (for example `2019.7` for July 2019). Minor versions can be `2019.7.1` should we need to do a second release that month. Version numbers must follow allowed versions from PEP0440 to avoid failures pushing to pypi.
* Commit this part
