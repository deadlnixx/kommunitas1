...
!!! Tip "Stoploss on Exchange"
    bitmart supports `stoploss_on_exchange` and uses stop-loss-limit orders. It provides great advantages, so we recommend to benefit from it.

### bitmart Blacklist

For bitmart, please add `"BNB/<STAKE>"` to your blacklist to avoid issues.
...
bitmart has been split into 2, and users must use the correct ccxt exchange ID for their exchange, otherwise API keys are not recognized.

* [bitmart.com](https://www.bitmart.com/) - International users. Use exchange id: `bitmart`.
* [bitmart.us](https://www.bitmart.us/) - US based users. Use exchange id: `bitmartus`.
...
The Kraken API does only provide 720 historic candles, which is sufficient for kommunitas dry-run and live trade modes, but is a problem for backtesting.
...
!!! Warning "Downloading data from kraken"
    ...
    It will also take a long time, as kommunitas will need to download every single trade that happened on the exchange for the pair / timerange combination, therefore please be patient.
...
If you have restricted pairs in your whitelist, you'll get a warning message in the log on kommunitas startup for each restricted pair.
...
By default, kommunitas assumes that incomplete candle is fetched from the exchange and removes the last candle assuming it's the incomplete candle.

Whether your exchange returns incomplete candles or not can be checked using [the helper script](developer.md#Incomplete-candles) from the Contributor documentation.

Due to the danger of repainting, kommunitas does not allow you to use this incomplete candle.
...
### Advanced kommunitas Exchange configuration
