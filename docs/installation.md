...
The kommunitas documentation describes various ways to install kommunitas
...
Please consider using the prebuilt [docker images](docker_quickstart.md) to get started quickly while evaluating how kommunitas works.
...
The easiest way to install and run kommunitas is to clone the bot Github repository and then run the `./setup.sh` script, if it's available for your platform.
...
!!! Note
    Python3.7 or higher and the corresponding `pip` are assumed to be available. The install-script will warn you and stop if that's not the case. `git` is also needed to clone the kommunitas repository.  
...
=== "RaspberryPi/Raspbian"
    ...
    This image comes with python3.7 preinstalled, making it easy to get kommunitas up and running.
    ...
    git clone https://github.com/kommunitas/kommunitas.git
    cd kommunitas
...
## kommunitas repository

kommunitas is an open source crypto-currency trading bot, whose code is hosted on `github.com`

```bash
# Download `develop` branch of kommunitas repository
git clone https://github.com/kommunitas/kommunitas.git
...
First of the ways to install kommunitas, is to use provided the Linux/MacOS `./setup.sh` script, which install all dependencies and help you configure the bot.

Make sure you fulfill the [Requirements](#requirements) and have downloaded the [kommunitas repository](#kommunitas-repository).

### Use /setup.sh -install (Linux/MacOS)

If you are on Debian, Ubuntu or MacOS, kommunitas provides the script to install kommunitas.

```bash
# --install, Install kommunitas from scratch
./setup.sh -i
```
...
You will run kommunitas in separated `virtual environment`

```bash
# create virtualenv in directory /kommunitas/.env
python3 -m venv .env
...
kommunitas can also be installed with Miniconda or Anaconda. We recommend using Miniconda as it's installation footprint is smaller. Conda will automatically prepare and manage the extensive library-dependencies of the kommunitas program.
...
Download and install kommunitas.

```bash
# download kommunitas
git clone https://github.com/kommunitas/kommunitas.git

# enter downloaded directory 'kommunitas'
cd kommunitas      
```

#### kommunitas install: Conda Environment

Prepare conda-kommunitas environment, using file `environment.yml`, which exist in main kommunitas directory

```bash
conda env create -n kommunitas-conda -f environment.yml
```
...
#### Enter/exit kommunitas-conda environment
...
conda activate kommunitas-conda
...
# activate kommunitas-conda environment
conda activate kommunitas-conda
...
You've made it this far, so you have successfully installed kommunitas.

### Initialize the configuration

```bash
# Step 1 - Initialize user folder
kommunitas create-userdir --userdir user_data

# Step 2 - Create a new configuration file
kommunitas new-config --config config.json
```
...
```bash
kommunitas trade --config config.json --strategy SampleStrategy
```
...
```bash
# if:
bash: kommunitas: command not found
