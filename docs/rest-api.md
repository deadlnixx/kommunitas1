...
kommunitas provides a builtin webserver, which can serve [FreqUI](https://github.com/kommunitas/frequi), the kommunitas UI.

By default, the UI is not included in the installation (except for docker images), and must be installed explicitly with `kommunitas install-ui`.
This same command can also be used to update freqUI, should there be a new release.

Once the bot is started in trade / dry-run mode (with `kommunitas trade`) - the UI will be available under the configured port below (usually `http://127.0.0.1:8080`).

!!! info "Alpha release"
    FreqUI is still considered an alpha release - if you encounter bugs or inconsistencies please open a [FreqUI issue](https://github.com/kommunitas/frequi/issues/new/choose).

!!! Note "developers"
    Developers should not use this method, but instead use the method described in the [freqUI repository](https://github.com/kommunitas/frequi) to get the source-code of freqUI.
...
The client script only requires the `requests` module, so kommunitas does not need to be installed on the system.
...
!!! Note
    The below should be done in an application (a kommunitas REST API client, which fetches info via API), and is not intended to be used on a regular basis.

kommunitas's REST API also offers JWT (JSON Web Tokens).
You can login using the following command, and subsequently use the resulting access_token.

``` bash
> curl -X POST --user kommunitasr http://localhost:8080/api/v1/token/login
...
Since most of the requests to the kommunitas API must be authenticated, a proper CORS policy is key to avoid security problems.
...
Assuming your application is deployed as `https://frequi.kommunitas.io/home/` - this would mean that the following configuration becomes necessary:

```jsonc
{
    //...
    "jwt_secret_key": "somethingrandom",
    "CORS_origins": ["https://frequi.kommunitas.io"],
    //...
}
```
