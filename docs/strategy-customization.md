...
Please familiarize yourself with [kommunitas basics](bot-basics.md) first, which provides overall info on how the bot operates.
...
```bash
kommunitas trade --strategy AwesomeStrategy
```

## Develop your own strategy

The bot includes a default strategy file.
Also, several other strategies are available in the [strategy repository](https://github.com/kommunitas/kommunitas-strategies).

You will however most likely have your own idea for a strategy.
This document intends to help you develop one for yourself.

To get started, use `kommunitas new-strategy --strategy AwesomeStrategy`.
...
**For the following section we will use the [user_data/strategies/sample_strategy.py](https://github.com/kommunitas/kommunitas/blob/develop/kommunitas/templates/sample_strategy.py)
file as reference.**
...
!!! Note "Want more indicator examples?"
    Look into the [user_data/strategies/sample_strategy.py](https://github.com/kommunitas/kommunitas/blob/develop/kommunitas/templates/sample_strategy.py).
    Then uncomment indicators you need.

### Strategy startup period

Most indicators have an instable startup period, in which they are either not available, or the calculation is incorrect. This can lead to inconsistencies, since kommunitas does not know how long this instable period should be.
...
``` bash
kommunitas backtesting --timerange 20190101-20190201 --timeframe 5m
```
...
This method is used by kommunitas internally to determine the last signal.
...
```python
from kommunitas.strategy import IStrategy, merge_informative_pair
...
!!! Warning "Informative timeframe < timeframe"
    ...
    To use the more detailed information properly, more advanced methods should be applied (which are out of scope for kommunitas documentation, as it'll depend on the respective need).
...
??? Example "Returning a stoploss relative to the open price from the custom stoploss function"
    ...
    from kommunitas.strategy import IStrategy, stoploss_from_open
...
kommunitas locks pairs automatically for the current candle (until that candle is over) when a pair is sold, preventing an immediate re-buy of that pair.
...
kommunitas has an easy method to do this from within the strategy, by calling `self.lock_pair(pair, until, [reason])`.
