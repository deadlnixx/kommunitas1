...
Creates the directory structure to hold your files for kommunitas.
...
```
usage: kommunitas create-userdir [-h] [--userdir PATH] [--reset]
...
```
usage: kommunitas new-config [-h] [-c PATH]
...
!!! Warning
    Only vital questions are asked. kommunitas offers a lot more configuration possibilities, which are listed in the [Configuration documentation](configuration.md#configuration-parameters)

### Create config examples

```
$ kommunitas new-config --config config_bitmart.json
...
``` output
usage: kommunitas new-strategy [-h] [--userdir PATH] [-s NAME]
...
```bash
kommunitas new-strategy --strategy AwesomeStrategy
```

With custom user directory

```bash
kommunitas new-strategy --userdir ~/.kommunitas/ --strategy AwesomeStrategy
```

Using the advanced template (populates all optional functions and methods)

```bash
kommunitas new-strategy --strategy AwesomeStrategy --template advanced
```
...
``` output
usage: kommunitas new-hyperopt [-h] [--userdir PATH] [--hyperopt NAME]
                              [--template {full,minimal,advanced}]
...
```bash
kommunitas new-hyperopt --hyperopt AwesomeHyperopt
```

With custom user directory

```bash
kommunitas new-hyperopt --userdir ~/.kommunitas/ --hyperopt AwesomeHyperopt
```
...
```
usage: kommunitas list-strategies [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                 [-d PATH] [--userdir PATH]
...
```
```
usage: kommunitas list-hyperopts [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                [-d PATH] [--userdir PATH]
...
``` bash
kommunitas list-strategies
kommunitas list-hyperopts
```

Example: Search strategies and hyperopts directory within the userdir.

``` bash
kommunitas list-strategies --userdir ~/.kommunitas/
kommunitas list-hyperopts --userdir ~/.kommunitas/
```

Example: Search dedicated strategy path.

``` bash
kommunitas list-strategies --strategy-path ~/.kommunitas/strategies/
```

Example: Search dedicated hyperopt path.

``` bash
kommunitas list-hyperopt --hyperopt-path ~/.kommunitas/hyperopts/
```
...
```
usage: kommunitas list-exchanges [-h] [-1] [-a]
...
```
$ kommunitas list-exchanges
Exchanges available for kommunitas:
...
* Example: see all exchanges supported by the ccxt library (including 'bad' ones, i.e. those that are known to not work with kommunitas):
```
$ kommunitas list-exchanges -a
...
bitstamp            False    Does not provide history. Details in https://github.com/kommunitas/kommunitas/issues/1983
...
```
usage: kommunitas list-timeframes [-h] [-v] [--logfile FILE] [-V] [-c PATH] [-d PATH] [--userdir PATH] [--exchange EXCHANGE] [-1]
...
* Example: see the timeframes for the 'bitmart' exchange, set in the configuration file:

```
$ kommunitas list-timeframes -c config_bitmart.json
...
Timeframes available for the exchange `bitmart`: 1m, 3m, 5m, 15m, 30m, 1h, 2h, 4h, 6h, 8h, 12h, 1d, 3d, 1w, 1M
```

* Example: enumerate exchanges available for kommunitas and print timeframes supported by each of them:
```
$ for i in `kommunitas list-exchanges -1`; do kommunitas list-timeframes --exchange $i; done
```
...
For pairs traded by kommunitas the pair quote currency is defined by the value of the `stake_currency` configuration setting.
...
```
usage: kommunitas list-markets [-h] [-v] [--logfile FILE] [-V] [-c PATH]
...
usage: kommunitas list-pairs [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                            [-d PATH] [--userdir PATH] [--exchange EXCHANGE]
                            [--print-list] [--print-json] [-1] [--print-csv]
                            [--base BASE_CURRENCY [BASE_CURRENCY ...]]
...
```
$ kommunitas list-pairs --quote USD --print-json
```

* Print the list of all pairs on the exchange, specified in the `config_bitmart.json` configuration file
(i.e. on the "bitmart" exchange) with base currencies BTC or ETH and quote currencies USDT or USD, as the
human-readable list with summary:

```
$ kommunitas list-pairs -c config_bitmart.json --all --base BTC ETH --quote USDT USD --print-list
```

* Print all markets on exchange "Kraken", in the tabular format:

```
$ kommunitas list-markets --exchange kraken --all
```
...
```
usage: kommunitas test-pairlist [-h] [-c PATH]
                               [--quote QUOTE_CURRENCY [QUOTE_CURRENCY ...]]
                               [-1] [--print-json]
...
```
kommunitas test-pairlist --config config.json --quote USDT BTC
```
...
Run kommunitas in webserver mode.
kommunitas will start the webserver and allow FreqUI to start and control backtesting processes.
...
```
usage: kommunitas webserver [-h] [-v] [--logfile FILE] [-V] [-c PATH] [-d PATH]
                           [--userdir PATH]
...
```
usage: kommunitas hyperopt-list [-h] [-v] [--logfile FILE] [-V] [-c PATH]
...
```
kommunitas hyperopt-list --profitable --no-details
```
...
```
usage: kommunitas hyperopt-show [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                               [-d PATH] [--userdir PATH] [--best]
...
```
kommunitas hyperopt-show -n 168
```

Prints JSON data with details for the last best epoch (i.e., the best of all epochs):

```
kommunitas hyperopt-show --best -n -1 --print-json --no-header
```
...
```
usage: kommunitas show-trades [-h] [-v] [--logfile FILE] [-V] [-c PATH]
...
``` bash
kommunitas show-trades --db-url sqlite:///tradesv3.sqlite --trade-ids 2 3 --print-json
```
