...
from kommunitas.data.btanalysis import (calculate_max_drawdown, combine_dataframes_with_mean,
                                       create_cum_profit, extract_trades_of_period, load_trades)
from kommunitas.data.converter import trim_dataframe
from kommunitas.data.dataprovider import DataProvider
from kommunitas.data.history import get_timerange, load_data
from kommunitas.exceptions import OperationalException
from kommunitas.exchange import timeframe_to_prev_date, timeframe_to_seconds
from kommunitas.misc import pair_to_filename
...
from kommunitas.strategy import IStrategy
...
def generate_profit_graph(pairs: str, data: Dict[str, pd.DataFrame],
    ...
    fig['layout'].update(title="kommunitas Profit plot")
    fig['layout']['yaxis1'].update(title='Price')
...
def generate_plot_filename(pair: str, timeframe: str) -> str:
    ...
    file_name = 'kommunitas-plot-' + pair_s + '-' + timeframe + '.html'
...
def plot_profit(config: Dict[str, Any]) -> None:
    ...
    store_plot_file(fig, filename='kommunitas-profit-plot.html',
                    directory=config['user_data_dir'] / 'plot',
