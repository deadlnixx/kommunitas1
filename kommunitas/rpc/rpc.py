...
from kommunitas.configuration.timerange import TimeRange
from kommunitas.constants import CANCEL_REASON, DATETIME_PRINT_FORMAT
from kommunitas.data.history import load_data
from kommunitas.enums import SellType, State
from kommunitas.exceptions import ExchangeError, PricingError
from kommunitas.exchange import timeframe_to_minutes, timeframe_to_msecs
from kommunitas.loggers import bufferHandler
from kommunitas.misc import decimals_per_coin, shorten_date
from kommunitas.persistence import PairLocks, Trade
from kommunitas.persistence.models import PairLock
from kommunitas.plugins.pairlist.pairlist_helpers import expand_pairlist
from kommunitas.rpc.fiat_convert import CryptoToFiatConverter
...
class RPC:
    ...
    def __init__(self, kommunitas) -> None:
        """
        Initializes all enabled rpc modules
        :param kommunitas: Instance of a kommunitas bot
        :return: None
        """
        self._kommunitas = kommunitas
        self._config: Dict[str, Any] = kommunitas.config
    ...
    def _rpc_show_config(config, botstate: Union[State, str]) -> Dict[str, Any]:
        ...
        val = {
            ...
            'bot_name': config.get('bot_name', 'kommunitas'),
        ...
        if not trades:
            ...
            for trade in trades:
                ...
                if trade.open_order_id:
                    order = self._kommunitas.exchange.fetch_order(trade.open_order_id, trade.pair)
                # calculate profit and send message to user
                if trade.is_open:
                    try:
                        current_rate = self._kommunitas.exchange.get_rate(
                ...
                if self._fiat_converter:
                    current_profit_fiat = self._fiat_converter.convert_amount(
                        current_profit_abs,
                        self._kommunitas.config['stake_currency'],
                        self._kommunitas.config['fiat_display_currency']
                    )
                ...
                trade_dict.update(dict(
                    base_currency=self._kommunitas.config['stake_currency'],
    ...
    def _rpc_trade_statistics(
        ...
        starting_balance = self._kommunitas.wallets.get_starting_balance()
    ...
    def _rpc_balance(self, stake_currency: str, fiat_display_currency: str) -> Dict:
        ...
        total = 0.0
        try:
            tickers = self._kommunitas.exchange.get_tickers(cached=True)
        except (ExchangeError):
            raise RPCException('Error getting current tickers.')

        self._kommunitas.wallets.update(require_update=False)

        for coin, balance in self._kommunitas.wallets.get_all_balances().items():
            ...
            if coin == stake_currency:
                ...
                try:
                    pair = self._kommunitas.exchange.get_valid_pair_combination(coin, stake_currency)
        ...
        if total == 0.0:
            if self._kommunitas.config['dry_run']:
        ...
        return {
            ...
            'value': value,
            'stake': stake_currency,
            'note': 'Simulated balances' if self._kommunitas.config['dry_run'] else ''
        }

    def _rpc_start(self) -> Dict[str, str]:
        """ Handler for start """
        if self._kommunitas.state == State.RUNNING:
            return {'status': 'already running'}

        self._kommunitas.state = State.RUNNING
        return {'status': 'starting trader ...'}

    def _rpc_stop(self) -> Dict[str, str]:
        """ Handler for stop """
        if self._kommunitas.state == State.RUNNING:
            self._kommunitas.state = State.STOPPED
    ...
    def _rpc_reload_config(self) -> Dict[str, str]:
        ...
        self._kommunitas.state = State.RELOAD_CONFIG
    ...
    def _rpc_stopbuy(self) -> Dict[str, str]:
        ...
        if self._kommunitas.state == State.RUNNING:
            ...
            self._kommunitas.config['max_open_trades'] = 0
    ...
    def _rpc_forcesell(self, trade_id: str) -> Dict[str, str]:
        ...
        def _exec_forcesell(trade: Trade) -> None:
            ...
            if trade.open_order_id:
                ...
                if order['side'] == 'buy':
                    fully_canceled = self._kommunitas.handle_cancel_buy(
                        trade, order, CANCEL_REASON['FORCE_SELL'])

                if order['side'] == 'sell':
                    # Cancel order - so it is placed anew with a fresh price.
                    self._kommunitas.handle_cancel_sell(trade, order, CANCEL_REASON['FORCE_SELL'])
            ...
            if not fully_canceled:
                ...
                self._kommunitas.execute_sell(trade, current_rate, sell_reason)
        # ---- EOF def _exec_forcesell ----

        if self._kommunitas.state != State.RUNNING:
            raise RPCException('trader is not running')

        with self._kommunitas._sell_lock:
            if trade_id == 'all':
                # Execute sell for all open orders
                for trade in Trade.get_open_trades():
                    _exec_forcesell(trade)
                Trade.commit()
                self._kommunitas.wallets.update()
    ...
    def _rpc_forcebuy(self, pair: str, price: Optional[float]) -> Optional[Trade]:
        ...
        if not self._kommunitas.config.get('forcebuy_enable', False):
        ...
        stake_currency = self._kommunitas.config.get('stake_currency')
        if not self._kommunitas.exchange.get_pair_quote_currency(pair) == stake_currency:
            raise RPCException(
        ...
        # gen stake amount
        stakeamount = self._kommunitas.wallets.get_trade_stake_amount(pair)

        # execute buy
        if self._kommunitas.execute_buy(pair, stakeamount, price, forcebuy=True):
            Trade.commit()
            trade = Trade.get_trades([Trade.is_open.is_(True), Trade.pair == pair]).first()
            return trade
        else:
    ...
    def _rpc_delete(self, trade_id: int) -> Dict[str, Union[str, int]]:
        ...
        with self._kommunitas._sell_lock:
            ...
            if trade.open_order_id:
                try:
                    self._kommunitas.exchange.cancel_order(trade.open_order_id, trade.pair)
                    c_count += 1
                except (ExchangeError):
                    pass

            # cancel stoploss on exchange ...
            if (self._kommunitas.strategy.order_types.get('stoploss_on_exchange')
                    and trade.stoploss_order_id):
                try:
                    self._kommunitas.exchange.cancel_stoploss_order(trade.stoploss_order_id,
                                                                   trade.pair)
                    c_count += 1
                except (ExchangeError):
                    pass
    ...
    def _rpc_count(self) -> Dict[str, float]:
        ...
        return {
            'current': len(trades),
            'max': (int(self._kommunitas.config['max_open_trades'])
                    if self._kommunitas.config['max_open_trades'] != float('inf') else -1),
    ...
    def _rpc_whitelist(self) -> Dict:
        ...
        res = {'method': self._kommunitas.pairlists.name_list,
               'length': len(self._kommunitas.active_pair_whitelist),
               'whitelist': self._kommunitas.active_pair_whitelist
               }
        return res
    ...
    def _rpc_blacklist(self, add: List[str] = None) -> Dict:
        ...
        if add:
            for pair in add:
                if pair not in self._kommunitas.pairlists.blacklist:
                    try:
                        expand_pairlist([pair], self._kommunitas.exchange.get_markets().keys())
                        self._kommunitas.pairlists.blacklist.append(pair)
        ...
        res = {'method': self._kommunitas.pairlists.name_list,
               'length': len(self._kommunitas.pairlists.blacklist),
               'blacklist': self._kommunitas.pairlists.blacklist,
               'blacklist_expanded': self._kommunitas.pairlists.expanded_blacklist,
               'errors': errors,
               }
        return res
    ...
    def _rpc_get_logs(limit: Optional[int]) -> Dict[str, Any]:
        ...
        #       "kommunitas.worker", "INFO", "Starting worker develop"]

        return {'log_count': len(records), 'logs': records}

    def _rpc_edge(self) -> List[Dict[str, Any]]:
        """ Returns information related to Edge """
        if not self._kommunitas.edge:
            raise RPCException('Edge is not enabled.')
        return self._kommunitas.edge.accepted_pairs()
    ...
    def _rpc_analysed_dataframe(self, pair: str, timeframe: str,
        ...
        _data, last_analyzed = self._kommunitas.dataprovider.get_analyzed_dataframe(
            pair, timeframe)
        _data = _data.copy()
        if limit:
            _data = _data.iloc[-limit:]
        return self._convert_dataframe_to_dict(self._kommunitas.config['strategy'],
    ...
    def _rpc_analysed_history_full(config, pair: str, timeframe: str,
        ...
        from kommunitas.data.dataprovider import DataProvider
    ...
    def _rpc_plot_config(self) -> Dict[str, Any]:
        if (self._kommunitas.strategy.plot_config and
                'subplots' not in self._kommunitas.strategy.plot_config):
            self._kommunitas.strategy.plot_config['subplots'] = {}
        return self._kommunitas.strategy.plot_config
