...
from kommunitas.exceptions import OperationalException, TemporaryError
...
class Worker:
    ...
    def _init(self, reconfig: bool) -> None:
        ...
        self.kommunitas = kommunitasBot(self._config)
    ...
    def _worker(self, old_state: Optional[State]) -> State:
        ...
        """
        state = self.kommunitas.state

        # Log state transition
        if state != old_state:
            self.kommunitas.notify_status(f'{state.name.lower()}')

            logger.info(f"Changing state to: {state.name}")
            if state == State.RUNNING:
                self.kommunitas.startup()

            if state == State.STOPPED:
                self.kommunitas.check_for_open_trades()
    ...
    def _process_stopped(self) -> None:
        self.kommunitas.process_stopped()

    def _process_running(self) -> None:
        try:
            self.kommunitas.process()
            ...
            self.kommunitas.notify_status(f'OperationalException:\n```\n{tb}```{hint}')

            logger.exception('OperationalException. Stopping trader ...')
            self.kommunitas.state = State.STOPPED

    def _reconfigure(self) -> None:
        """
        Cleans up current kommunitasbot instance, reloads the configuration and
        replaces it with the new instance
        """
        # Tell systemd that we initiated reconfiguration
        self._notify("RELOADING=1")

        # Clean up current kommunitas modules
        self.kommunitas.cleanup()

        # Load and validate config and create new instance of the bot
        self._init(True)

        self.kommunitas.notify_status('config reloaded')
    ...
    def exit(self) -> None:
        ...
        if self.kommunitas:
            self.kommunitas.notify_status('process died')
            self.kommunitas.cleanup()
