...
function updateenv() {
    ...
    if [ $? -ne 0 ]; then
        echo "Failed installing kommunitas"
        exit 1
    fi
...
function config() {
    ...
    echo "-------------------------"
    echo "Please use 'kommunitas new-config -c config.json' to generate a new configuration file."
    echo "-------------------------"
}
...
function install() {
    ...
    echo "Run the bot !"
    echo "-------------------------"
    echo "You can now use the bot by executing 'source .env/bin/activate; kommunitas <subcommand>'."
    echo "You can see the list of available bot sub-commands by executing 'source .env/bin/activate; kommunitas --help'."
    echo "You verify that kommunitas is installed successfully by running 'source .env/bin/activate; kommunitas --version'."
}
...
function help() {
    echo "usage:"
    echo "	-i,--install    Install kommunitas from scratch"
