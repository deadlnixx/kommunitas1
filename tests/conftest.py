...
from kommunitas.persistence import LocalTrade, Trade, init_db
from kommunitas.resolvers import ExchangeResolver
...
def log_has(line, logs):
    # caplog mocker returns log as a tuple: ('kommunitas.something', logging.WARNING, 'foobar')
...
def patched_configuration_load_config_file(mocker, config) -> None:
    mocker.patch(
        'kommunitas.configuration.configuration.load_config_file',
        lambda *args, **kwargs: config
    )


def patch_exchange(mocker, api_mock=None, id='bitmart', mock_markets=True) -> None:
    mocker.patch('kommunitas.exchange.Exchange._load_async_markets', MagicMock(return_value={}))
    mocker.patch('kommunitas.exchange.Exchange.validate_pairs', MagicMock())
    mocker.patch('kommunitas.exchange.Exchange.validate_timeframes', MagicMock())
    mocker.patch('kommunitas.exchange.Exchange.validate_ordertypes', MagicMock())
    mocker.patch('kommunitas.exchange.Exchange.validate_stakecurrency', MagicMock())
    mocker.patch('kommunitas.exchange.Exchange.id', PropertyMock(return_value=id))
    mocker.patch('kommunitas.exchange.Exchange.name', PropertyMock(return_value=id.title()))
    mocker.patch('kommunitas.exchange.Exchange.precisionMode', PropertyMock(return_value=2))
    if mock_markets:
        mocker.patch('kommunitas.exchange.Exchange.markets',
                     PropertyMock(return_value=get_markets()))

    if api_mock:
        mocker.patch('kommunitas.exchange.Exchange._init_ccxt', MagicMock(return_value=api_mock))
    else:
        mocker.patch('kommunitas.exchange.Exchange._init_ccxt', MagicMock())


def get_patched_exchange(mocker, config, api_mock=None, id='bitmart',
...
def patch_wallet(mocker, free=999.9) -> None:
    mocker.patch('kommunitas.wallets.Wallets.get_free', MagicMock(
        return_value=free
    ))


def patch_whitelist(mocker, conf) -> None:
    mocker.patch('kommunitas.kommunitasbot.kommunitasBot._refresh_active_whitelist',
...
def patch_edge(mocker) -> None:
    ...
    mocker.patch('kommunitas.edge.Edge.calculate', MagicMock(return_value=True))
...
def patch_kommunitasbot(mocker, config) -> None:
    """
    ...
    :return: None
    """
    mocker.patch('kommunitas.kommunitasbot.RPCManager', MagicMock())
    init_db(config['db_url'])
    patch_exchange(mocker)
    mocker.patch('kommunitas.kommunitasbot.RPCManager._init', MagicMock())
    mocker.patch('kommunitas.kommunitasbot.RPCManager.send_msg', MagicMock())
    patch_whitelist(mocker, config)


def get_patched_kommunitasbot(mocker, config) -> kommunitasBot:
    """
    This function patches _init_modules() to not call dependencies
    :param mocker: a Mocker object to apply patches
    :param config: Config to pass to the bot
    :return: kommunitasBot
    """
    patch_kommunitasbot(mocker, config)
    config['datadir'] = Path(config['datadir'])
    return kommunitasBot(config)
...
def patch_get_signal(kommunitas: kommunitasBot, value=(True, False, None)) -> None:
    """
    :param mocker: mocker to patch IStrategy class
    :param value: which value IStrategy.get_signal() must return
    :return: None
    """
    kommunitas.strategy.get_signal = lambda e, s, x: value
    kommunitas.exchange.refresh_latest_ohlcv = lambda p: None
...
def patch_coingekko(mocker) -> None:
    ...
    mocker.patch.multiple(
        'kommunitas.rpc.fiat_convert.CoinGeckoAPI',
