...
from kommunitas.exceptions import (DDosProtection, DependencyException, InvalidOrderException,
                                  OperationalException, PricingError, TemporaryError)
from kommunitas.exchange import bitmart, Bittrex, Exchange, Kraken
from kommunitas.exchange.common import (API_FETCH_ORDER_RETRY_COUNT, API_RETRY_COUNT,
                                       calculate_backoff)
from kommunitas.exchange.exchange import (market_is_active, timeframe_to_minutes, timeframe_to_msecs,
...
EXCHANGES = ['bittrex', 'bitmart', 'kraken', 'ftx']


def ccxt_exceptionhandlers(mocker, default_conf, api_mock, exchange_name,
                           fun, mock_ccxt_fun, retries=API_RETRY_COUNT + 1, **kwargs):

    with patch('kommunitas.exchange.common.time.sleep'):
...
async def async_ccxt_exception(mocker, default_conf, api_mock, fun, mock_ccxt_fun,
    ...
    with patch('kommunitas.exchange.common.asyncio.sleep', get_mock_coro(None)):
        with pytest.raises(DDosProtection):
...
def test_init_ccxt_kwargs(default_conf, mocker, caplog):
    mocker.patch('kommunitas.exchange.Exchange._load_markets', MagicMock(return_value={}))
    mocker.patch('kommunitas.exchange.Exchange.validate_stakecurrency')
    caplog.set_level(logging.INFO)
...
def test_init_exception(default_conf, mocker):
    ...
    with pytest.raises(OperationalException,
                       match=f"Exchange {default_conf['exchange']['name']} is not supported"):
        mocker.patch("ccxt.bitmart", MagicMock(side_effect=AttributeError))
        Exchange(default_conf)

    with pytest.raises(OperationalException,
                       match=r"Initialization of ccxt failed. Reason: DeadBeef"):
        mocker.patch("ccxt.bitmart", MagicMock(side_effect=ccxt.BaseError("DeadBeef")))
        Exchange(default_conf)


def test_exchange_resolver(default_conf, mocker, caplog):
    mocker.patch('kommunitas.exchange.Exchange._init_ccxt', MagicMock(return_value=MagicMock()))
    mocker.patch('kommunitas.exchange.Exchange._load_async_markets')
    mocker.patch('kommunitas.exchange.Exchange.validate_pairs')
    mocker.patch('kommunitas.exchange.Exchange.validate_timeframes')
    mocker.patch('kommunitas.exchange.Exchange.validate_stakecurrency')
    ...
    assert not isinstance(exchange, bitmart)
    assert not log_has_re(r"No .* specific subclass found. Using the generic class instead.",
                          caplog)

    exchange = ExchangeResolver.load_exchange('bitmart', default_conf)
    assert isinstance(exchange, Exchange)
    assert isinstance(exchange, bitmart)
    ...
    # Test mapping
    exchange = ExchangeResolver.load_exchange('bitmartus', default_conf)
    assert isinstance(exchange, Exchange)
    assert isinstance(exchange, bitmart)
    assert not isinstance(exchange, Kraken)
...
def test_amount_to_precision(default_conf, mocker, amount, precision_mode, precision, expected):
    ...
    exchange = get_patched_exchange(mocker, default_conf, id="bitmart")
    # digits counting mode
    # DECIMAL_PLACES = 2
    # SIGNIFICANT_DIGITS = 3
    # TICK_SIZE = 4
    mocker.patch('kommunitas.exchange.Exchange.precisionMode',
                 PropertyMock(return_value=precision_mode))
    mocker.patch('kommunitas.exchange.Exchange.markets', markets)
...
def test_get_min_pair_stake_amount(mocker, default_conf) -> None:
    ...
    # no pair found
    mocker.patch(
        'kommunitas.exchange.Exchange.markets',
...
def test__load_async_markets(default_conf, mocker, caplog):
    mocker.patch('kommunitas.exchange.Exchange._init_ccxt')
    mocker.patch('kommunitas.exchange.Exchange.validate_pairs')
    mocker.patch('kommunitas.exchange.Exchange.validate_timeframes')
    mocker.patch('kommunitas.exchange.Exchange._load_markets')
    mocker.patch('kommunitas.exchange.Exchange.validate_stakecurrency')
...
def test__load_markets(default_conf, mocker, caplog):
    ...
    mocker.patch('kommunitas.exchange.Exchange._init_ccxt', MagicMock(return_value=api_mock))
    mocker.patch('kommunitas.exchange.Exchange.validate_pairs')
    mocker.patch('kommunitas.exchange.Exchange.validate_timeframes')
    mocker.patch('kommunitas.exchange.Exchange._load_async_markets')
    mocker.patch('kommunitas.exchange.Exchange.validate_stakecurrency')
...
def test_reload_markets(default_conf, mocker, caplog):
    ...
    exchange = get_patched_exchange(mocker, default_conf, api_mock, id="bitmart",
                                    mock_markets=False)
...
def test_validate_pairs_exception(default_conf, mocker, caplog):
    caplog.set_level(logging.INFO)
    api_mock = MagicMock()
    mocker.patch('kommunitas.exchange.Exchange.name', PropertyMock(return_value='bitmart'))

    type(api_mock).markets = PropertyMock(return_value={})
    mocker.patch('kommunitas.exchange.Exchange._init_ccxt', api_mock)
    mocker.patch('kommunitas.exchange.Exchange.validate_timeframes')
    mocker.patch('kommunitas.exchange.Exchange.validate_stakecurrency')
    mocker.patch('kommunitas.exchange.Exchange._load_async_markets')

    with pytest.raises(OperationalException, match=r'Pair ETH/BTC is not available on bitmart'):
        Exchange(default_conf)

    mocker.patch('kommunitas.exchange.Exchange.markets', PropertyMock(return_value={}))
    Exchange(default_conf)
...
def test_validate_timeframes_failed(default_conf, mocker):
    ...
    mocker.patch('kommunitas.exchange.Exchange._init_ccxt', MagicMock(return_value=api_mock))
    mocker.patch('kommunitas.exchange.Exchange._load_markets', MagicMock(return_value={}))
    mocker.patch('kommunitas.exchange.Exchange.validate_pairs', MagicMock())
    with pytest.raises(OperationalException,
    ...
    with pytest.raises(OperationalException,
                       match=r"Timeframes < 1m are currently not supported by kommunitas."):
        Exchange(default_conf)
...
def test_validate_order_types(default_conf, mocker):
    ...
    mocker.patch('kommunitas.exchange.Exchange.validate_timeframes')
    mocker.patch('kommunitas.exchange.Exchange.validate_stakecurrency')
    mocker.patch('kommunitas.exchange.Exchange.name', 'Bittrex')
...
def test_create_dry_run_order_limit_fill(default_conf, mocker, side, startprice, endprice,
    ...
    default_conf['dry_run'] = True
    exchange = get_patched_exchange(mocker, default_conf, id=exchange_name)
    mocker.patch.multiple('kommunitas.exchange.Exchange',
...
def test_create_order(default_conf, mocker, side, ordertype, rate, marketprice, exchange_name):
    ...
    mocker.patch('kommunitas.exchange.Exchange.amount_to_precision', lambda s, x, y: y)
    mocker.patch('kommunitas.exchange.Exchange.price_to_precision', lambda s, x, y: y)
    exchange = get_patched_exchange(mocker, default_conf, api_mock, id=exchange_name)
...
async def test___async_get_candle_history_sort(default_conf, mocker, exchange_name):
    ...
    exchange = get_patched_exchange(mocker, default_conf, id=exchange_name)
    exchange._api_async.fetch_ohlcv = get_mock_coro(ohlcv)
    sort_mock = mocker.patch('kommunitas.exchange.exchange.sorted', MagicMock(side_effect=sort_data))
    ...
    exchange._api_async.fetch_ohlcv = get_mock_coro(ohlcv)
    # Reset sort mock
    sort_mock = mocker.patch('kommunitas.exchange.sorted', MagicMock(side_effect=sort_data))
    # Test the OHLCV data sort
    res = await exchange._async_get_candle_history('ETH/BTC', default_conf['timeframe'])
    assert res[0] == 'ETH/BTC'
...
def test_get_historic_trades(default_conf, mocker, caplog, exchange_name, trades_history):
    mocker.patch('kommunitas.exchange.Exchange.exchange_has', return_value=True)
    exchange = get_patched_exchange(mocker, default_conf, id=exchange_name)
...
def test_get_historic_trades_notsupported(default_conf, mocker, caplog, exchange_name,
                                          trades_history):
    mocker.patch('kommunitas.exchange.Exchange.exchange_has', return_value=False)
    exchange = get_patched_exchange(mocker, default_conf, id=exchange_name)
...
def test_cancel_order_dry_run(default_conf, mocker, exchange_name):
    default_conf['dry_run'] = True
    exchange = get_patched_exchange(mocker, default_conf, id=exchange_name)
    mocker.patch('kommunitas.exchange.Exchange._is_dry_limit_order_filled', return_value=True)
...
def test_cancel_stoploss_order_with_result(default_conf, mocker, exchange_name):
    default_conf['dry_run'] = False
    mocker.patch('kommunitas.exchange.Exchange.fetch_stoploss_order', return_value={'for': 123})
    mocker.patch('kommunitas.exchange.Ftx.fetch_stoploss_order', return_value={'for': 123})
    exchange = get_patched_exchange(mocker, default_conf, id=exchange_name)

    mocker.patch('kommunitas.exchange.Exchange.cancel_stoploss_order',
                 return_value={'fee': {}, 'status': 'canceled', 'amount': 1234})
    mocker.patch('kommunitas.exchange.Ftx.cancel_stoploss_order',
                 return_value={'fee': {}, 'status': 'canceled', 'amount': 1234})
    ...
    mocker.patch('kommunitas.exchange.Exchange.fetch_stoploss_order',
                 side_effect=InvalidOrderException(""))
    mocker.patch('kommunitas.exchange.Ftx.fetch_stoploss_order',
                 side_effect=InvalidOrderException(""))
...
def test_fetch_order(default_conf, mocker, exchange_name, caplog):
    ...
    exchange = get_patched_exchange(mocker, default_conf, api_mock, id=exchange_name)
    with patch('kommunitas.exchange.common.time.sleep') as tm:
        with pytest.raises(InvalidOrderException):
            exchange.fetch_order(order_id='_', pair='TKN/BTC')
...
def test_get_trades_for_order(default_conf, mocker, exchange_name):
    ...
    mocker.patch('kommunitas.exchange.Exchange.exchange_has', MagicMock(return_value=False))
...
def test_merge_ft_has_dict(default_conf, mocker):
    ...
    # bitmart defines different values
    ex = bitmart(default_conf)
    assert ex._ft_has != Exchange._ft_has_default
...
def test_get_markets_error(default_conf, mocker):
    ex = get_patched_exchange(mocker, default_conf)
    mocker.patch('kommunitas.exchange.Exchange.markets', PropertyMock(return_value=None))
...
@pytest.mark.parametrize("market_symbol,base,quote,exchange,add_dict,expected_result", [
    ("BTC/USDT", 'BTC', 'USDT', "bitmart", {}, True),
    ("USDT/BTC", 'USDT', 'BTC', "bitmart", {}, True),
    ("USDT/BTC", 'BTC', 'USDT', "bitmart", {}, False),  # Reversed currencies
    ("BTCUSDT", 'BTC', 'USDT', "bitmart", {}, False),  # No seperating /
    ("BTCUSDT", None, "USDT", "bitmart", {}, False),  #
    ("USDT/BTC", "BTC", None, "bitmart", {}, False),
    ("BTCUSDT", "BTC", None, "bitmart", {}, False),
    ("BTC/USDT", "BTC", "USDT", "bitmart", {}, True),
    ("BTC/USDT", "USDT", "BTC", "bitmart", {}, False),  # reversed currencies
    ("BTC/USDT", "BTC", "USD", "bitmart", {}, False),  # Wrong quote currency
    ("BTC/", "BTC", 'UNK', "bitmart", {}, False),
    ("/USDT", 'UNK', 'USDT', "bitmart", {}, False),
...
def test_extract_cost_curr_rate(mocker, default_conf, order, expected) -> None:
    mocker.patch('kommunitas.exchange.Exchange.calculate_fee_rate', MagicMock(return_value=0.01))
    ex = get_patched_exchange(mocker, default_conf)
...
def test_calculate_fee_rate(mocker, default_conf, order, expected) -> None:
    mocker.patch('kommunitas.exchange.Exchange.fetch_ticker', return_value={'last': 0.081})
