...
from kommunitas.commands.optimize_commands import setup_optimize_configuration, start_backtesting
from kommunitas.configuration import TimeRange
from kommunitas.data import history
from kommunitas.data.btanalysis import BT_DATA_COLUMNS, evaluate_result_multi
from kommunitas.data.converter import clean_ohlcv_dataframe
from kommunitas.data.dataprovider import DataProvider
from kommunitas.data.history import get_timerange
from kommunitas.enums import RunMode, SellType
from kommunitas.exceptions import DependencyException, OperationalException
from kommunitas.optimize.backtesting import Backtesting
from kommunitas.persistence import LocalTrade
...
def test_setup_bt_configuration_with_arguments(mocker, default_conf, caplog) -> None:
    patched_configuration_load_config_file(mocker, default_conf)
    mocker.patch(
        'kommunitas.configuration.configuration.create_datadir',
        lambda c, x: x
    )
...
def test_start(mocker, fee, default_conf, caplog) -> None:
    ...
    mocker.patch('kommunitas.exchange.Exchange.get_fee', fee)
    patch_exchange(mocker)
    mocker.patch('kommunitas.optimize.backtesting.Backtesting.start', start_mock)
    patched_configuration_load_config_file(mocker, default_conf)
    ...
    assert log_has('Starting kommunitas in Backtesting mode', caplog)
...
def test_backtesting_init(mocker, default_conf, order_types) -> None:
    ...
    patch_exchange(mocker)
    get_fee = mocker.patch('kommunitas.exchange.Exchange.get_fee', MagicMock(return_value=0.5))
...
def test_backtesting_init_no_timeframe(mocker, default_conf, caplog) -> None:
    ...
    mocker.patch('kommunitas.exchange.Exchange.get_fee', MagicMock(return_value=0.5))
...
def test_data_with_fee(default_conf, mocker, testdatadir) -> None:
    ...
    fee_mock = mocker.patch('kommunitas.exchange.Exchange.get_fee', MagicMock(return_value=0.5))
    backtesting = Backtesting(default_conf)
    backtesting._set_strategy(backtesting.strategylist[0])
...
def test_backtesting_start(default_conf, mocker, testdatadir, caplog) -> None:
    ...
    mocker.patch('kommunitas.data.history.get_timerange', get_timerange)
    patch_exchange(mocker)
    mocker.patch('kommunitas.optimize.backtesting.Backtesting.backtest')
    mocker.patch('kommunitas.optimize.backtesting.generate_backtest_stats')
    mocker.patch('kommunitas.optimize.backtesting.show_backtest_results')
    sbs = mocker.patch('kommunitas.optimize.backtesting.store_backtest_stats')
    mocker.patch('kommunitas.plugins.pairlistmanager.PairListManager.whitelist',
...
def test_backtesting_start_no_data(default_conf, mocker, caplog, testdatadir) -> None:
    ...
    mocker.patch('kommunitas.data.history.history_utils.load_pair_history',
...
def test_backtesting_pairlist_list(default_conf, mocker, caplog, testdatadir, tickers) -> None:
    ...
    mocker.patch('kommunitas.plugins.pairlistmanager.PairListManager.refresh_pairlist')
...
def test_backtest__enter_trade(default_conf, fee, mocker) -> None:
    ...
    mocker.patch('kommunitas.exchange.Exchange.get_fee', fee)
    mocker.patch("kommunitas.exchange.Exchange.get_min_pair_stake_amount", return_value=0.00001)
    patch_exchange(mocker)
    ...
    mocker.patch("kommunitas.exchange.Exchange.get_min_pair_stake_amount", return_value=600.0)

    trade = backtesting._enter_trade(pair, row=row)
    assert trade is None

    # Stake-amount throwing error
    mocker.patch("kommunitas.wallets.Wallets.get_trade_stake_amount",
...
def test_backtest_start_multi_strat(default_conf, mocker, caplog, testdatadir):
    ...
    mocker.patch('kommunitas.plugins.pairlistmanager.PairListManager.whitelist',
                 PropertyMock(return_value=['UNITTEST/BTC']))
    mocker.patch('kommunitas.optimize.backtesting.Backtesting.backtest', backtestmock)
    text_table_mock = MagicMock()
    sell_reason_mock = MagicMock()
    strattable_mock = MagicMock()
    strat_summary = MagicMock()

    mocker.patch.multiple('kommunitas.optimize.optimize_reports',
