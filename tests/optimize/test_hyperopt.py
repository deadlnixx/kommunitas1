...
from kommunitas.commands.optimize_commands import setup_optimize_configuration, start_hyperopt
...
from kommunitas.optimize.hyperopt import Hyperopt
from kommunitas.optimize.hyperopt_auto import HyperOptAuto
from kommunitas.optimize.hyperopt_tools import HyperoptTools
from kommunitas.optimize.optimize_reports import generate_strategy_stats
from kommunitas.optimize.space import SKDecimal
from kommunitas.resolvers.hyperopt_resolver import HyperOptResolver
from kommunitas.strategy.hyper import IntParameter
...
def test_hyperoptresolver(mocker, default_conf, caplog) -> None:
    ...
    mocker.patch(
        'kommunitas.resolvers.hyperopt_resolver.HyperOptResolver.load_object',
...
def test_start_not_installed(mocker, default_conf, import_fails) -> None:
    ...
    mocker.patch('kommunitas.optimize.hyperopt.Hyperopt.start', start_mock)
...
def test_start(mocker, hyperopt_conf, caplog) -> None:
    ...
    assert log_has('Starting kommunitas in Hyperopt mode', caplog)
    assert start_mock.call_count == 1


def test_start_no_data(mocker, hyperopt_conf) -> None:
    patched_configuration_load_config_file(mocker, hyperopt_conf)
    mocker.patch('kommunitas.data.history.load_pair_history', MagicMock(return_value=pd.DataFrame))
    mocker.patch(
        'kommunitas.optimize.hyperopt.get_timerange',
...
def test_start_filelock(mocker, hyperopt_conf, caplog) -> None:
    ...
    patched_configuration_load_config_file(mocker, hyperopt_conf)
    mocker.patch('kommunitas.optimize.hyperopt.Hyperopt.__init__', hyperopt_mock)
    patch_exchange(mocker)
    ...
    pargs = get_args(args)
    start_hyperopt(pargs)
    assert log_has("Another running instance of kommunitas Hyperopt detected.", caplog)
...
def test_start_calls_optimizer(mocker, hyperopt_conf, capsys) -> None:
    dumper = mocker.patch('kommunitas.optimize.hyperopt.dump')
    dumper2 = mocker.patch('kommunitas.optimize.hyperopt.Hyperopt._save_result')
    mocker.patch('kommunitas.optimize.hyperopt.file_dump_json')

    mocker.patch('kommunitas.optimize.backtesting.Backtesting.load_bt_data',
    ...
    parallel = mocker.patch(
        'kommunitas.optimize.hyperopt.Hyperopt.run_optimizer_parallel',
...
def test_generate_optimizer(mocker, hyperopt_conf) -> None:
    ...
    mocker.patch('kommunitas.optimize.hyperopt.Backtesting.backtest', return_value=backtest_result)
    mocker.patch('kommunitas.optimize.hyperopt.get_timerange',
                 return_value=(Arrow(2017, 12, 10), Arrow(2017, 12, 13)))
    patch_exchange(mocker)
    mocker.patch.object(Path, 'open')
    mocker.patch('kommunitas.optimize.hyperopt.load', return_value={'XRP/BTC': None})
...
def test_clean_hyperopt(mocker, hyperopt_conf, caplog):
    ...
    mocker.patch("kommunitas.strategy.hyper.HyperStrategyMixin.load_params_from_file",
                 MagicMock(return_value={}))
    mocker.patch("kommunitas.optimize.hyperopt.Path.is_file", MagicMock(return_value=True))
    unlinkmock = mocker.patch("kommunitas.optimize.hyperopt.Path.unlink", MagicMock())
...
def test_simplified_interface_all_failed(mocker, hyperopt_conf) -> None:
    mocker.patch('kommunitas.optimize.hyperopt.dump', MagicMock())
    mocker.patch('kommunitas.optimize.hyperopt.file_dump_json')
    mocker.patch('kommunitas.optimize.backtesting.Backtesting.load_bt_data',
                 MagicMock(return_value=(MagicMock(), None)))
    mocker.patch(
