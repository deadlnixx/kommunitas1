...
from kommunitas.constants import AVAILABLE_PAIRLISTS
...
from kommunitas.plugins.pairlistmanager import PairListManager
from kommunitas.resolvers import PairListResolver
from tests.conftest import get_patched_exchange, get_patched_kommunitasbot, log_has, log_has_re
...
def test_log_cached(mocker, static_pl_conf, markets, tickers):
    ...
    kommunitas = get_patched_kommunitasbot(mocker, static_pl_conf)
    logmock = MagicMock()
    # Assign starting whitelist
    pl = kommunitas.pairlists._pairlist_handlers[0]
...
def test_load_pairlist_noexist(mocker, markets, default_conf):
    ...
    plm = PairListManager(kommunitas.exchange, default_conf)
    with pytest.raises(OperationalException,
                       match=r"Impossible to load Pairlist 'NonexistingPairList'. "
                             r"This class does not exist or contains Python code errors."):
        PairListResolver.load_pairlist('NonexistingPairList', kommunitas.exchange, plm,
                                       default_conf, {}, 1)
...
def test_refresh_market_pair_not_in_whitelist(mocker, markets, static_pl_conf):
    ...
    mocker.patch('kommunitas.exchange.Exchange.markets', PropertyMock(return_value=markets))
    kommunitas.pairlists.refresh_pairlist()
    # List ordered by BaseVolume
    whitelist = ['ETH/BTC', 'TKN/BTC']
    # Ensure all except those in whitelist are removed
    assert set(whitelist) == set(kommunitas.pairlists.whitelist)
    # Ensure config dict hasn't been changed
    assert (static_pl_conf['exchange']['pair_whitelist'] ==
            kommunitas.config['exchange']['pair_whitelist'])
...
def test_refresh_static_pairlist(mocker, markets, static_pl_conf):
    ...
    # Ensure all except those in whitelist are removed
    assert set(whitelist) == set(kommunitas.pairlists.whitelist)
    assert static_pl_conf['exchange']['pair_blacklist'] == kommunitas.pairlists.blacklist
...
def test_refresh_static_pairlist_noexist(mocker, markets, static_pl_conf, pairs, expected, caplog):
    ...
    # Ensure all except those in whitelist are removed
    assert set(expected) == set(kommunitas.pairlists.whitelist)
    assert static_pl_conf['exchange']['pair_blacklist'] == kommunitas.pairlists.blacklist
    if not expected:
...
def test_refresh_pairlist_dynamic(mocker, shitcoinmarkets, tickers, whitelist_conf):
    ...
    kommunitas = get_patched_kommunitasbot(mocker, whitelist_conf)
    # Remock markets with shitcoinmarkets since get_patched_kommunitasbot uses the markets fixture
    mocker.patch.multiple(
        'kommunitas.exchange.Exchange',
    ...
    kommunitas.pairlists.refresh_pairlist()
    assert whitelist == kommunitas.pairlists.whitelist

    whitelist_conf['pairlists'] = [{'method': 'VolumePairList'}]
    with pytest.raises(OperationalException,
                       match=r'`number_assets` not specified. Please check your configuration '
                             r'for "pairlist.config.number_assets"'):
        PairListManager(kommunitas.exchange, whitelist_conf)
...
def test_refresh_pairlist_dynamic_2(mocker, shitcoinmarkets, tickers, whitelist_conf_2):
    ...
    mocker.patch.multiple(
        'kommunitas.plugins.pairlistmanager.PairListManager',
        _get_cached_tickers=MagicMock(return_value=tickers_dict),
    )
    kommunitas = get_patched_kommunitasbot(mocker, whitelist_conf_2)
    # Remock markets with shitcoinmarkets since get_patched_kommunitasbot uses the markets fixture
    mocker.patch.multiple(
        'kommunitas.exchange.Exchange',
        markets=PropertyMock(return_value=shitcoinmarkets),
...
def test_VolumePairList_refresh_empty(mocker, markets_empty, whitelist_conf):
    ...
    kommunitas = get_patched_kommunitasbot(mocker, whitelist_conf)
    mocker.patch('kommunitas.exchange.Exchange.markets', PropertyMock(return_value=markets_empty))
...
def test_VolumePairList_whitelist_gen(mocker, whitelist_conf, shitcoinmarkets, tickers,
    ...
    mocker.patch.multiple('kommunitas.persistence.Trade',
    ...
    if whitelist_result == 'filter_at_the_beginning':
        ...
        kommunitas.pairlists.refresh_pairlist()
        whitelist = kommunitas.pairlists.whitelist
...
def test_gen_pair_whitelist_not_supported(mocker, default_conf, tickers) -> None:
    ...
    with pytest.raises(OperationalException,
        ...
        get_patched_kommunitasbot(mocker, default_conf)
...
def test_pairlist_class(mocker, whitelist_conf, markets, pairlist):
    ...
    assert kommunitas.pairlists.name_list == [pairlist]
    assert pairlist in str(kommunitas.pairlists.short_desc())
    assert isinstance(kommunitas.pairlists.whitelist, list)
    assert isinstance(kommunitas.pairlists.blacklist, list)
...
@pytest.mark.parametrize("whitelist,log_message", [
    ...
    (['ETH/BTC', 'TKN/BTC', 'XLTCUSDT'], "is not tradable with kommunitas"),
])
def test__whitelist_for_active_markets(mocker, whitelist_conf, markets, pairlist, whitelist, caplog,
    ...
    # Assign starting whitelist
    pairlist_handler = kommunitas.pairlists._pairlist_handlers[0]
...
def test_volumepairlist_invalid_sortvalue(mocker, whitelist_conf):
    ...
    with pytest.raises(OperationalException,
        ...
        get_patched_kommunitasbot(mocker, whitelist_conf)
...
def test_volumepairlist_caching(mocker, markets, whitelist_conf, tickers):
    ...
    kommunitas = get_patched_kommunitasbot(mocker, whitelist_conf)
    assert len(kommunitas.pairlists._pairlist_handlers[0]._pair_cache) == 0
    assert tickers.call_count == 0
    kommunitas.pairlists.refresh_pairlist()
    assert tickers.call_count == 1

    assert len(kommunitas.pairlists._pairlist_handlers[0]._pair_cache) == 1
    kommunitas.pairlists.refresh_pairlist()
    assert tickers.call_count == 1
...
def test_agefilter_caching(mocker, markets, whitelist_conf_agefilter, tickers, ohlcv_history):
    ...
    kommunitas = get_patched_kommunitasbot(mocker, whitelist_conf_agefilter)
    assert kommunitas.exchange.refresh_latest_ohlcv.call_count == 0
    kommunitas.pairlists.refresh_pairlist()
    assert len(kommunitas.pairlists.whitelist) == 3
    assert kommunitas.exchange.refresh_latest_ohlcv.call_count > 0

    previous_call_count = kommunitas.exchange.refresh_latest_ohlcv.call_count
    kommunitas.pairlists.refresh_pairlist()
    assert len(kommunitas.pairlists.whitelist) == 3
    # Called once for XRP/BTC
    assert kommunitas.exchange.refresh_latest_ohlcv.call_count == previous_call_count + 1
...
def test_rangestabilityfilter_caching(mocker, markets, default_conf, tickers, ohlcv_history,
    ...
    assert kommunitas.exchange.refresh_latest_ohlcv.call_count == 0
    kommunitas.pairlists.refresh_pairlist()
    assert len(kommunitas.pairlists.whitelist) == expected_length
    assert kommunitas.exchange.refresh_latest_ohlcv.call_count > 0
    ...
    assert kommunitas.exchange.refresh_latest_ohlcv.call_count == previous_call_count
...
def test_spreadfilter_invalid_data(mocker, default_conf, markets, tickers, caplog):
    ...
    mocker.patch.multiple('kommunitas.exchange.Exchange', get_tickers=tickers)
...
def test_pricefilter_desc(mocker, whitelist_conf, markets, pairlistconfig,
    ...
    if desc_expected is not None:
        kommunitas = get_patched_kommunitasbot(mocker, whitelist_conf)
        short_desc = str(kommunitas.pairlists.short_desc())
...
def test_performance_filter(mocker, whitelist_conf, pairlists, pair_allowlist, overall_performance,
    ...
    kommunitas = get_patched_kommunitasbot(mocker, allowlist_conf)
    mocker.patch.multiple('kommunitas.exchange.Exchange',
                          get_tickers=tickers,
                          markets=PropertyMock(return_value=markets)
                          )
    ...
    kommunitas.pairlists.refresh_pairlist()
    allowlist = kommunitas.pairlists.whitelist
