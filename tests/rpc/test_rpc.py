...
from kommunitas.enums import State
from kommunitas.exceptions import ExchangeError, InvalidOrderException, TemporaryError
from kommunitas.persistence import Trade
from kommunitas.persistence.pairlock_middleware import PairLocks
from kommunitas.rpc import RPC, RPCException
from kommunitas.rpc.fiat_convert import CryptoToFiatConverter
from tests.conftest import create_mock_trades, get_patched_kommunitasbot, patch_get_signal
...
def test_rpc_trade_status(default_conf, ticker, fee, mocker) -> None:
    ...
    kommunitasbot.exit_positions(trades)
...
def test_rpc_delete_trade(mocker, default_conf, fee, markets, caplog):
    ...
    kommunitasbot = get_patched_kommunitasbot(mocker, default_conf)
    kommunitasbot.strategy.order_types['stoploss_on_exchange'] = True
    create_mock_trades(fee)
    rpc = RPC(kommunitasbot)
    ...
    stoploss_mock = mocker.patch('kommunitas.exchange.Exchange.cancel_stoploss_order',
    ...
    cancel_mock = mocker.patch('kommunitas.exchange.Exchange.cancel_order',
                               side_effect=InvalidOrderException)
...
def test_rpc_forcesell(default_conf, ticker, fee, mocker) -> None:
    ...
    mocker.patch('kommunitas.wallets.Wallets.get_free', return_value=1000)
    ...
    mocker.patch(
        'kommunitas.exchange.Exchange._is_dry_limit_order_filled', MagicMock(return_value=False))
    kommunitasbot.enter_positions()
    ...
    mocker.patch(
        'kommunitas.exchange.Exchange.fetch_order',
        side_effect=[{
            'id': '1234',
            'status': 'open',
            'type': 'limit',
    ...
    kommunitasbot.config['max_open_trades'] = 3
    kommunitasbot.enter_positions()
...
def test_rpcforcebuy(mocker, default_conf, ticker, fee, limit_buy_order_open) -> None:
    ...
    # Test not buying
    kommunitasbot = get_patched_kommunitasbot(mocker, default_conf)
    kommunitasbot.config['stake_amount'] = 0
    patch_get_signal(kommunitasbot, (True, False, None))
    rpc = RPC(kommunitasbot)
    pair = 'TKN/BTC'
    trade = rpc._rpc_forcebuy(pair, None)
...
def test_rpc_edge_enabled(mocker, edge_conf) -> None:
    ...
    kommunitasbot = get_patched_kommunitasbot(mocker, edge_conf)
