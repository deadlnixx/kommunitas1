...
from kommunitas.__init__ import __version__
from kommunitas.enums import RunMode, State
from kommunitas.exceptions import DependencyException, ExchangeError, OperationalException
from kommunitas.loggers import setup_logging, setup_logging_pre
...
from kommunitas.rpc.api_server.api_auth import create_token, get_user_from_token
from kommunitas.rpc.api_server.uvicorn_threaded import UvicornServer
from tests.conftest import (create_mock_trades, get_mock_coro, get_patched_kommunitasbot, log_has,
                            log_has_re, patch_get_signal)


BASE_URI = "/api/v1"
_TEST_USER = "kommunitasr"
...
def botclient(default_conf, mocker):
    ...
    rpc = RPC(ftbot)
    mocker.patch('kommunitas.rpc.api_server.ApiServer.start_api', MagicMock())
    try:
...
def test_api_ui_fallback(botclient):
    ...
    assert rc.status_code == 200
    assert '`kommunitas install-ui`' in rc.text
...
def test_api_ui_version(botclient, mocker):
    ...
    mocker.patch('kommunitas.commands.deploy_commands.read_ui_version', return_value='0.1.2')
...
def test_api_auth():
    with pytest.raises(ValueError):
        create_token({'identity': {'u': 'kommunitas'}}, 'secret1234', token_type="NotATokenType")

    token = create_token({'identity': {'u': 'kommunitas'}}, 'secret1234')
...
def test_api__init__(default_conf, mocker):
    ...
    mocker.patch('kommunitas.rpc.api_server.webserver.ApiServer.start_api', MagicMock())
    apiserver = ApiServer(default_conf)
    apiserver.add_rpc_handler(RPC(get_patched_kommunitasbot(mocker, default_conf)))
...
def test_api_UvicornServer(mocker):
    thread_mock = mocker.patch('kommunitas.rpc.api_server.uvicorn_threaded.threading.Thread')
...
def test_api_UvicornServer_run(mocker):
    serve_mock = mocker.patch('kommunitas.rpc.api_server.uvicorn_threaded.UvicornServer.serve',
                              get_mock_coro(None))
    s = UvicornServer(uvicorn.Config(MagicMock(), port=8080, host='127.0.0.1'))
...
def test_api_run(default_conf, mocker, caplog):
    ...
    mocker.patch('kommunitas.rpc.api_server.webserver.UvicornServer', server_mock)
    ...
    caplog.clear()
    mocker.patch('kommunitas.rpc.api_server.webserver.UvicornServer',
...
def test_api_show_config(botclient, mocker):
    ...
    assert rc.json()['exchange'] == 'bitmart'
    assert rc.json()['timeframe'] == '5m'
    assert rc.json()['timeframe_ms'] == 300000
    assert rc.json()['timeframe_min'] == 5
    assert rc.json()['state'] == 'running'
    assert rc.json()['bot_name'] == 'kommunitas'
...
def test_api_status(botclient, mocker, ticker, fee, markets):
    ...
    mocker.patch('kommunitas.exchange.Exchange.get_rate',
...
def test_api_backtesting(botclient, mocker, fee, caplog):
    ...
    mocker.patch('kommunitas.optimize.backtesting.Backtesting.backtest_one_strategy',
