...
from kommunitas import __version__
from kommunitas.constants import CANCEL_REASON
from kommunitas.edge import PairInfo
from kommunitas.enums import RPCMessageType, RunMode, SellType, State
from kommunitas.exceptions import OperationalException
from kommunitas.kommunitasbot import kommunitasBot
from kommunitas.loggers import setup_logging
from kommunitas.persistence import PairLocks, Trade
from kommunitas.rpc import RPC
from kommunitas.rpc.telegram import Telegram, authorized_only
from tests.conftest import (create_mock_trades, get_patched_kommunitasbot, log_has, log_has_re,
...
def get_telegram_testobject(mocker, default_conf, mock=True, ftbot=None):
    ...
    if mock:
        mocker.patch.multiple(
            'kommunitas.rpc.telegram.Telegram',
            _init=MagicMock(),
            _send_msg=msg_mock
        )
    if not ftbot:
...
def test_telegram__init__(default_conf, mocker) -> None:
    mocker.patch('kommunitas.rpc.telegram.Updater', MagicMock())
    mocker.patch('kommunitas.rpc.telegram.Telegram._init', MagicMock())
...
def test_telegram_init(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch('kommunitas.rpc.telegram.Updater', MagicMock(return_value=start_polling))
...
def test_cleanup(default_conf, mocker, ) -> None:
    ...
    mocker.patch('kommunitas.rpc.telegram.Updater', updater_mock)
...
def test_authorized_only(default_conf, mocker, caplog, update) -> None:
    ...
    bot = kommunitasBot(default_conf)
    rpc = RPC(bot)
...
def test_telegram_status(default_conf, update, mocker) -> None:
    ...
    mocker.patch('kommunitas.rpc.telegram.Telegram._status_table', status_table)

    mocker.patch.multiple(
        'kommunitas.rpc.rpc.RPC',
...
def test_status_handle(default_conf, update, ticker, fee, mocker) -> None:
    ...
    telegram, kommunitasbot, msg_mock = get_telegram_testobject(mocker, default_conf)

    patch_get_signal(kommunitasbot, (True, False, None))

    kommunitasbot.state = State.STOPPED
    ...
    kommunitasbot.state = State.RUNNING
    telegram._status(update=update, context=MagicMock())
    assert msg_mock.call_count == 1
    assert 'no active trade' in msg_mock.call_args_list[0][0][0]
    msg_mock.reset_mock()
...
def test_daily_handle(default_conf, update, ticker, limit_buy_order, fee,
    ...
    mocker.patch(
        'kommunitas.rpc.rpc.CryptoToFiatConverter._find_price',
    ...
    # Reset msg_mock
    msg_mock.reset_mock()
    kommunitasbot.config['max_open_trades'] = 2
    # Add two other trades
    n = kommunitasbot.enter_positions()
    assert n == 2
...
def test_profit_handle(default_conf, update, ticker, ticker_sell_up, fee,
    ...
    mocker.patch('kommunitas.rpc.rpc.CryptoToFiatConverter._find_price', return_value=15000.0)
    mocker.patch.multiple(
        'kommunitas.exchange.Exchange',
        fetch_ticker=ticker,
        get_fee=fee,
    ...
    mocker.patch('kommunitas.wallets.Wallets.get_starting_balance', return_value=0.01)
    assert ('∙ `-0.00000500 BTC (-0.50%) (-0.0 \N{GREEK CAPITAL LETTER SIGMA}%)`'
            in msg_mock.call_args_list[-1][0][0])
    msg_mock.reset_mock()

    # Update the ticker with a market going up
    mocker.patch('kommunitas.exchange.Exchange.fetch_ticker', ticker_sell_up)
    trade.update(limit_sell_order)
...
def test_telegram_balance_handle(default_conf, update, mocker, rpc_balance, tickers) -> None:
    ...
    mocker.patch('kommunitas.exchange.Exchange.get_balances', return_value=rpc_balance)
    mocker.patch('kommunitas.exchange.Exchange.get_tickers', tickers)
    mocker.patch('kommunitas.exchange.Exchange.get_valid_pair_combination',
...
def test_balance_handle_empty_response(default_conf, update, mocker) -> None:
    default_conf['dry_run'] = False
    mocker.patch('kommunitas.exchange.Exchange.get_balances', return_value={})

    telegram, kommunitasbot, msg_mock = get_telegram_testobject(mocker, default_conf)
    patch_get_signal(kommunitasbot, (True, False, None))

    kommunitasbot.config['dry_run'] = False
    telegram._balance(update=update, context=MagicMock())
    result = msg_mock.call_args_list[0][0][0]
    assert msg_mock.call_count == 1
...
def test_balance_handle_too_large_response(default_conf, update, mocker) -> None:
    ...
    mocker.patch('kommunitas.rpc.rpc.RPC._rpc_balance', return_value={
        'currencies': balances,
        'total': 100.0,
        'symbol': 100.0,
        'value': 1000.0,
...
def test_start_handle(default_conf, update, mocker) -> None:
    ...
    kommunitasbot.state = State.STOPPED
    assert kommunitasbot.state == State.STOPPED
    telegram._start(update=update, context=MagicMock())
    assert kommunitasbot.state == State.RUNNING
    assert msg_mock.call_count == 1
...
def test_stopbuy_handle(default_conf, update, mocker) -> None:
    ...
    assert kommunitasbot.config['max_open_trades'] != 0
...
def test_reload_config_handle(default_conf, update, mocker) -> None:
    ...
    assert kommunitasbot.state == State.RELOAD_CONFIG
    assert msg_mock.call_count == 1
...
def test_telegram_forcesell_handle(default_conf, update, ticker, fee,
    ...
    mocker.patch('kommunitas.rpc.rpc.CryptoToFiatConverter._find_price', return_value=15000.0)
    msg_mock = mocker.patch('kommunitas.rpc.telegram.Telegram.send_msg', MagicMock())
    mocker.patch('kommunitas.rpc.telegram.Telegram._init', MagicMock())
    patch_exchange(mocker)
    ...
    kommunitasbot = kommunitasBot(default_conf)
    rpc = RPC(kommunitasbot)
    telegram = Telegram(rpc, default_conf)
    patch_get_signal(kommunitasbot, (True, False, None))
...
def test_telegram_forcesell_down_handle(default_conf, update, ticker, fee,
    ...
    mocker.patch('kommunitas.rpc.fiat_convert.CryptoToFiatConverter._find_price',
                 return_value=15000.0)
    msg_mock = mocker.patch('kommunitas.rpc.telegram.Telegram.send_msg', MagicMock())
    mocker.patch('kommunitas.rpc.telegram.Telegram._init', MagicMock())
    patch_exchange(mocker)
...
def test_forcebuy_handle(default_conf, update, mocker) -> None:
    ...
    mocker.patch('kommunitas.rpc.RPC._rpc_forcebuy', fbuy_mock)

    telegram, kommunitasbot, _ = get_telegram_testobject(mocker, default_conf)
    patch_get_signal(kommunitasbot, (True, False, None))
...
def test_blacklist_static(default_conf, update, mocker) -> None:
    ...
    assert ("Blacklist contains 3 pairs\n`DOGE/BTC, HOT/BTC, ETH/BTC`"
            in msg_mock.call_args_list[0][0][0])
    assert kommunitasbot.pairlists.blacklist == ["DOGE/BTC", "HOT/BTC", "ETH/BTC"]
    ...
    assert ("Blacklist contains 4 pairs\n`DOGE/BTC, HOT/BTC, ETH/BTC, XRP/.*`"
            in msg_mock.call_args_list[0][0][0])
    assert kommunitasbot.pairlists.blacklist == ["DOGE/BTC", "HOT/BTC", "ETH/BTC", "XRP/.*"]
...
def test_telegram_logs(default_conf, update, mocker) -> None:
    ...
    assert msg_mock.call_count == 1
    assert "kommunitas\\.rpc\\.telegram" in msg_mock.call_args_list[0][0][0]
    ...
    mocker.patch('kommunitas.rpc.telegram.MAX_TELEGRAM_MESSAGE_LENGTH', 200)
    context = MagicMock()
    context.args = []
    telegram._logs(update=update, context=context)
...
def test_edge_enabled(edge_conf, update, mocker) -> None:
    mocker.patch('kommunitas.edge.Edge._cached_pairs', mocker.PropertyMock(
        return_value={
...
def test_show_config_handle(default_conf, update, mocker) -> None:
    ...
    assert '*Exchange:* `bitmart`' in msg_mock.call_args_list[0][0][0]
    assert '*Strategy:* `DefaultStrategy`' in msg_mock.call_args_list[0][0][0]
    assert '*Stoploss:* `-0.1`' in msg_mock.call_args_list[0][0][0]

    msg_mock.reset_mock()
    kommunitasbot.config['trailing_stop'] = True
    telegram._show_config(update=update, context=MagicMock())
    assert msg_mock.call_count == 1
    assert '*Mode:* `{}`'.format('Dry-run') in msg_mock.call_args_list[0][0][0]
    assert '*Exchange:* `bitmart`' in msg_mock.call_args_list[0][0][0]
...
def test_send_msg_buy_notification(default_conf, mocker, caplog) -> None:
    ...
    assert msg_mock.call_args[0][0] \
        == '\N{LARGE BLUE CIRCLE} *bitmart:* Buying ETH/BTC (#1)\n' \
    ...
    kommunitasbot.config['telegram']['notification_settings'] = {'buy': 'off'}
    caplog.clear()
    msg_mock.reset_mock()
    telegram.send_msg(msg)
    ...
    kommunitasbot.config['telegram']['notification_settings'] = {'buy': 'silent'}
    caplog.clear()
    msg_mock.reset_mock()
...
def test_send_msg_buy_cancel_notification(default_conf, mocker) -> None:
    ...
    assert (msg_mock.call_args[0][0] == '\N{WARNING SIGN} *bitmart:* '
...
def test_send_msg_buy_fill_notification(default_conf, mocker) -> None:
    ...
    assert (msg_mock.call_args[0][0] == '\N{LARGE CIRCLE} *bitmart:* '
...
def test_send_msg_sell_notification(default_conf, mocker) -> None:
    ...
    assert msg_mock.call_args[0][0] \
        == ('\N{WARNING SIGN} *bitmart:* Selling KEY/ETH (#1)\n'
...
def test_send_msg_sell_cancel_notification(default_conf, mocker) -> None:
    ...
    assert msg_mock.call_args[0][0] \
        == ('\N{WARNING SIGN} *bitmart:* Cancelling open sell Order for KEY/ETH (#1).'
            ' Reason: Cancelled on exchange.')
...
def test_send_msg_sell_fill_notification(default_conf, mocker) -> None:
    ...
    assert msg_mock.call_args[0][0] \
        == ('\N{LARGE CIRCLE} *bitmart:* Sell order for ETH/USDT (#1) filled for 550.')
...
def test_send_msg_buy_notification_no_fiat(default_conf, mocker) -> None:
    ...
    assert msg_mock.call_args[0][0] == ('\N{LARGE BLUE CIRCLE} *bitmart:* Buying ETH/BTC (#1)\n'
                                        '*Buy Tag:* `buy_signal_01`\n'
                                        '*Amount:* `1333.33333333`\n'
                                        '*Open Rate:* `0.00001099`\n'
                                        '*Current Rate:* `0.00001099`\n'
...
def test_send_msg_sell_notification_no_fiat(default_conf, mocker) -> None:
    ...
    assert msg_mock.call_args[0][0] == ('\N{WARNING SIGN} *bitmart:* Selling KEY/ETH (#1)\n'
                                        '*Profit:* `-57.41%`\n'
                                        '*Sell Reason:* `stop_loss`\n'
...
def test__send_msg_keyboard(default_conf, mocker, caplog) -> None:
    ...
    kommunitasbot = get_patched_kommunitasbot(mocker, default_conf)
    rpc = RPC(kommunitasbot)
    ...
    def init_telegram(kommunitasbot):
        telegram = Telegram(rpc, default_conf)
        telegram._updater = MagicMock()
        telegram._updater.bot = bot
        return telegram
    ...
    kommunitasbot.config['telegram']['enabled'] = True
    telegram = init_telegram(kommunitasbot)
    telegram._send_msg('test')
    ...
    kommunitasbot.config['telegram']['enabled'] = True
    kommunitasbot.config['telegram']['keyboard'] = invalid_keys_list
    ...
    kommunitasbot.config['telegram']['enabled'] = True
    kommunitasbot.config['telegram']['keyboard'] = custom_keys_list
    telegram = init_telegram(kommunitasbot)
    telegram._send_msg('test')
    used_keyboard = bot.send_message.call_args[1]['reply_markup']
