...
from kommunitas.configuration import (Configuration, check_exchange, remove_credentials,
                                     validate_config_consistency)
from kommunitas.configuration.config_validation import validate_config_schema
from kommunitas.configuration.deprecated_settings import (check_conflicting_settings,
                                                         process_deprecated_setting,
                                                         process_removed_setting,
                                                         process_temporary_deprecated_settings)
from kommunitas.configuration.environment_vars import flat_vars_to_nested_dict
from kommunitas.configuration.load_config import load_config_file, load_file, log_config_error_range
from kommunitas.constants import DEFAULT_DB_DRYRUN_URL, DEFAULT_DB_PROD_URL, ENV_VAR_PREFIX
from kommunitas.enums import RunMode
from kommunitas.exceptions import OperationalException
from kommunitas.loggers import _set_loggers, setup_logging, setup_logging_pre
...
def test_load_config_file(default_conf, mocker, caplog) -> None:
    ...
    file_mock = mocker.patch('kommunitas.configuration.load_config.open', mocker.mock_open(
...
def test_load_config_file_error(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch('kommunitas.configuration.load_config.open', mocker.mock_open(read_data=filedata))
...
def test_from_config(default_conf, mocker, caplog) -> None:
    ...
    config_files = [conf1, conf2]
    mocker.patch('kommunitas.configuration.configuration.create_datadir', lambda c, x: x)

    configsmock = MagicMock(side_effect=config_files)
    mocker.patch('kommunitas.configuration.configuration.load_config_file', configsmock)
...
def test_load_config_file_exception(mocker) -> None:
    mocker.patch(
        'kommunitas.configuration.configuration.open',
...
def test_setup_configuration_with_arguments(mocker, default_conf, caplog) -> None:
    ...
    mocker.patch(
        'kommunitas.configuration.configuration.create_userdata_dir',
    ...
    assert log_has('Using user-data directory: {} ...'.format(Path("/tmp/kommunitas")), caplog)
    assert 'user_data_dir' in config
...
def test_check_exchange(default_conf, caplog) -> None:
    # Test an officially supported by kommunitas team exchange
    default_conf['runmode'] = RunMode.DRY_RUN
    default_conf.get('exchange').update({'name': 'BITTREX'})
    assert check_exchange(default_conf)
    assert log_has_re(r"Exchange .* is officially supported by the kommunitas development team\.",
                      caplog)
    caplog.clear()

    # Test an officially supported by kommunitas team exchange
    default_conf.get('exchange').update({'name': 'bitmart'})
    assert check_exchange(default_conf)
    assert log_has_re(r"Exchange .* is officially supported by the kommunitas development team\.",
                      caplog)
    caplog.clear()
    ...
    assert log_has_re(r"Exchange .* is known to the the ccxt library, available for the bot, "
                      ...
                      r"by the kommunitas development team\. .*", caplog)
    caplog.clear()

    # Test a 'bad' exchange, which known to have serious problems
    default_conf.get('exchange').update({'name': 'bitmex'})
    with pytest.raises(OperationalException,
                       match=r"Exchange .* will not work with kommunitas\..*"):
...
def test_cli_verbose_with_params(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch('kommunitas.loggers._set_loggers', MagicMock)
    arglist = ['trade', '-vvv']
    args = Arguments(arglist).get_parsed_arg()
...
def test_pairlist_resolving():
    ...
    assert config['exchange']['name'] == 'bitmart'
...
def test_pairlist_resolving_fallback(mocker):
    ...
    mocker.patch("kommunitas.configuration.configuration.load_file",
    ...
    assert config['pairs'] == ['ETH/BTC', 'XRP/BTC']
    assert config['exchange']['name'] == 'bitmart'
    assert config['datadir'] == Path.cwd() / "user_data/data/bitmart"
...
def test_flat_vars_to_nested_dict(caplog):
    ...
    test_args = {
        'kommunitas__EXCHANGE__SOME_SETTING': 'true',
        'kommunitas__EXCHANGE__SOME_FALSE_SETTING': 'false',
        'kommunitas__EXCHANGE__CONFIG__whatever': 'sometime',
        'kommunitas__ASK_STRATEGY__PRICE_SIDE': 'bid',
        'kommunitas__ASK_STRATEGY__cccc': '500',
        'kommunitas__STAKE_AMOUNT': '200.05',
    ...
    assert log_has("Loading variable 'kommunitas__EXCHANGE__SOME_SETTING'", caplog)
