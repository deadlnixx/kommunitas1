...
from kommunitas.constants import CANCEL_REASON, MATH_CLOSE_PREC, UNLIMITED_STAKE_AMOUNT
from kommunitas.enums import RPCMessageType, RunMode, SellType, State
from kommunitas.exceptions import (DependencyException, ExchangeError, InsufficientFundsError,
                                  InvalidOrderException, OperationalException, PricingError,
                                  TemporaryError)
from kommunitas.kommunitasbot import kommunitasBot
from kommunitas.persistence import Order, PairLocks, Trade
from kommunitas.persistence.models import PairLock
from kommunitas.strategy.interface import SellCheckTuple
from kommunitas.worker import Worker
from tests.conftest import (create_mock_trades, get_patched_kommunitasbot, get_patched_worker,
...
def patch_RPCManager(mocker) -> MagicMock:
    ...
    rpc_mock = mocker.patch('kommunitas.kommunitasbot.RPCManager.send_msg', MagicMock())
    return rpc_mock


# Unit tests

def test_kommunitasbot_state(mocker, default_conf, markets) -> None:
    mocker.patch('kommunitas.exchange.Exchange.markets', PropertyMock(return_value=markets))
    kommunitas = get_patched_kommunitasbot(mocker, default_conf)
    assert kommunitas.state is State.RUNNING

    default_conf.pop('initial_state')
    kommunitas = kommunitasBot(default_conf)
    assert kommunitas.state is State.STOPPED


def test_process_stopped(mocker, default_conf) -> None:

    kommunitas = get_patched_kommunitasbot(mocker, default_conf)
    coo_mock = mocker.patch('kommunitas.kommunitasbot.kommunitasBot.cancel_all_open_orders')
    kommunitas.process_stopped()
...
def test_bot_cleanup(mocker, default_conf, caplog) -> None:
    mock_cleanup = mocker.patch('kommunitas.kommunitasbot.cleanup_db')
    coo_mock = mocker.patch('kommunitas.kommunitasbot.kommunitasBot.cancel_all_open_orders')
    kommunitas = get_patched_kommunitasbot(mocker, default_conf)
    kommunitas.cleanup()
    ...
    kommunitas.config['cancel_open_orders_on_exit'] = True
    kommunitas.cleanup()
    assert coo_mock.call_count == 1
...
def test_order_dict_dry_run(default_conf, mocker, caplog) -> None:
    ...
    kommunitas = kommunitasBot(conf)
    assert kommunitas.strategy.order_types['stoploss_on_exchange']
    ...
    kommunitas = kommunitasBot(conf)
    assert not kommunitas.strategy.order_types['stoploss_on_exchange']
...
def test_get_trade_stake_amount(default_conf, ticker, mocker) -> None:
    ...
    result = kommunitas.wallets.get_trade_stake_amount('ETH/BTC')
...
def test_check_available_stake_amount(default_conf, ticker, mocker, fee, limit_buy_order_open,
    ...
    for i in range(0, max_open):
        ...
        if expected[i] is not None:
            ...
            kommunitas.execute_buy('ETH/BTC', result)
        else:
...
def test_edge_called_in_process(mocker, edge_conf) -> None:
    ...
    patch_exchange(mocker)
    kommunitas = kommunitasBot(edge_conf)
    kommunitas.pairlists._validate_whitelist = _refresh_whitelist
    patch_get_signal(kommunitas)
    kommunitas.process()
    assert kommunitas.active_pair_whitelist == ['NEO/BTC', 'LTC/BTC']
...
def test_edge_overrides_stake_amount(mocker, edge_conf) -> None:
    ...
    assert kommunitas.wallets.get_trade_stake_amount(
        'NEO/BTC', kommunitas.edge) == (999.9 * 0.5 * 0.01) / 0.20
    assert kommunitas.wallets.get_trade_stake_amount(
        'LTC/BTC', kommunitas.edge) == (999.9 * 0.5 * 0.01) / 0.21
...
def test_edge_overrides_stoploss(limit_buy_order, fee, caplog, mocker, edge_conf) -> None:
    ...
    kommunitas = kommunitasBot(edge_conf)
    kommunitas.active_pair_whitelist = ['NEO/BTC']
    patch_get_signal(kommunitas)
    kommunitas.strategy.min_roi_reached = MagicMock(return_value=False)
    ...
    assert kommunitas.handle_trade(trade) is True
...
def test_edge_should_ignore_strategy_stoploss(limit_buy_order, fee,
    ...
    assert kommunitas.handle_trade(trade) is False
...
def test_create_trade(default_conf, ticker, limit_buy_order, fee, mocker) -> None:
    ...
    kommunitas = kommunitasBot(default_conf)
    patch_get_signal(kommunitas)
    kommunitas.create_trade('ETH/BTC')
...
def test_create_trade_too_small_stake_amount(default_conf, ticker, limit_buy_order_open,
    ...
    kommunitas = kommunitasBot(default_conf)
    kommunitas.config['stake_amount'] = 0.000000005

    patch_get_signal(kommunitas)

    assert kommunitas.create_trade('ETH/BTC')
...
def test_create_trade_zero_stake_amount(default_conf, ticker, limit_buy_order_open,
    ...
    kommunitas = kommunitasBot(default_conf)
    kommunitas.config['stake_amount'] = 0

    patch_get_signal(kommunitas)

    assert not kommunitas.create_trade('ETH/BTC')
...
def test_create_trade_limit_reached(default_conf, ticker, limit_buy_order_open,
    ...
    assert not kommunitas.create_trade('ETH/BTC')
    assert kommunitas.wallets.get_trade_stake_amount('ETH/BTC', kommunitas.edge) == 0
...
def test_create_trade_no_signal(default_conf, fee, mocker) -> None:
    ...
    kommunitas = kommunitasBot(default_conf)
    patch_get_signal(kommunitas, value=(False, False, None))
...
def test_create_trades_preopen(default_conf, ticker, fee, mocker, limit_buy_order_open) -> None:
    ...
    kommunitas.execute_buy('ETH/BTC', default_conf['stake_amount'])
    kommunitas.execute_buy('NEO/BTC', default_conf['stake_amount'])
    ...
    assert kommunitas.create_trade('ETH/BTC')
    assert kommunitas.create_trade('NEO/BTC')
...
def test_process_exchange_failures(default_conf, ticker, mocker) -> None:
    ...
    patch_get_signal(worker.kommunitas)
...
def test_process_operational_exception(default_conf, ticker, mocker) -> None:
    ...
    assert worker.kommunitas.state == State.RUNNING

    worker._process_running()
    assert worker.kommunitas.state == State.STOPPED
...
def test_process_trade_no_whitelist_pair(default_conf, ticker, limit_buy_order,
    ...
    assert pair not in kommunitas.active_pair_whitelist
    kommunitas.process()
    assert pair in kommunitas.active_pair_whitelist
    # Make sure each pair is only in the list once
    assert len(kommunitas.active_pair_whitelist) == len(set(kommunitas.active_pair_whitelist))
...
def test_process_informative_pairs_added(default_conf, ticker, mocker) -> None:
    ...
    mocker.patch(
        'kommunitas.strategy.interface.IStrategy.get_signal',
    ...
    kommunitas = kommunitasBot(default_conf)
    kommunitas.pairlists._validate_whitelist = _refresh_whitelist
    kommunitas.strategy.informative_pairs = inf_pairs
    # patch_get_signal(kommunitas)
...
def test_execute_buy(mocker, default_conf, fee, limit_buy_order, limit_buy_order_open) -> None:
    ...
    patch_exchange(mocker)
    kommunitas = kommunitasBot(default_conf)
    kommunitas.strategy.confirm_trade_entry = MagicMock(return_value=False)
    stake_amount = 2
    bid = 0.11
    ...
    fix_price = 0.06
    assert kommunitas.execute_buy(pair, stake_amount, fix_price)
    ...
    mocker.patch('kommunitas.exchange.Exchange.create_order',
    ...
    kommunitas.strategy.custom_stake_amount = lambda **kwargs: 150.0
    assert kommunitas.execute_buy(pair, stake_amount)
    ...
    kommunitas.strategy.custom_stake_amount = lambda **kwargs: 20 / 0
    assert kommunitas.execute_buy(pair, stake_amount)
    ...
    # Fail to get price...
    mocker.patch('kommunitas.exchange.Exchange.get_rate', MagicMock(return_value=0.0))

    with pytest.raises(PricingError, match="Could not determine buy price."):
        kommunitas.execute_buy(pair, stake_amount)
...
def test_execute_buy_confirm_error(mocker, default_conf, fee, limit_buy_order) -> None:
    ...
    kommunitas.strategy.confirm_trade_entry = MagicMock(side_effect=ValueError)
    assert kommunitas.execute_buy(pair, stake_amount)

    limit_buy_order['id'] = '222'
    kommunitas.strategy.confirm_trade_entry = MagicMock(side_effect=Exception)
    assert kommunitas.execute_buy(pair, stake_amount)
...
def test_add_stoploss_on_exchange(mocker, default_conf, limit_buy_order) -> None:
    patch_RPCManager(mocker)
    patch_exchange(mocker)
    mocker.patch('kommunitas.kommunitasbot.kommunitasBot.handle_trade', MagicMock(return_value=True))
    mocker.patch('kommunitas.exchange.Exchange.fetch_order', return_value=limit_buy_order)
    mocker.patch('kommunitas.exchange.Exchange.get_trades_for_order', return_value=[])
    mocker.patch('kommunitas.kommunitasbot.kommunitasBot.get_real_amount',
    ...
    kommunitas.exit_positions(trades)
...
def test_handle_stoploss_on_exchange(mocker, default_conf, fee, caplog,
    ...
    assert kommunitas.handle_stoploss_on_exchange(trade) is False
    assert stoploss.call_count == 1
    assert trade.stoploss_order_id == "13434334"
    ...
    mocker.patch('kommunitas.exchange.bitmart.fetch_stoploss_order', hanging_stoploss_order)
    ...
    mocker.patch('kommunitas.exchange.bitmart.fetch_stoploss_order', canceled_stoploss_order)
    stoploss.reset_mock()
    ...
    mocker.patch('kommunitas.exchange.bitmart.fetch_stoploss_order', stoploss_order_hit)
    assert kommunitas.handle_stoploss_on_exchange(trade) is True
    assert log_has_re(r'STOP_LOSS_LIMIT is hit for Trade\(id=1, .*\)\.', caplog)
    assert trade.stoploss_order_id is None
    assert trade.is_open is False

    mocker.patch(
        'kommunitas.exchange.bitmart.stoploss',
        side_effect=ExchangeError()
    )
    trade.is_open = True
    kommunitas.handle_stoploss_on_exchange(trade)
    ...
    trade.stoploss_order_id = 100
    stoploss.reset_mock()
    mocker.patch('kommunitas.exchange.bitmart.fetch_stoploss_order',
    ...
    trade.is_open = False
    stoploss.reset_mock()
    mocker.patch('kommunitas.exchange.Exchange.fetch_order')
    mocker.patch('kommunitas.exchange.bitmart.stoploss', stoploss)
    assert kommunitas.handle_stoploss_on_exchange(trade) is False
...
def test_create_stoploss_order_invalid_order(mocker, default_conf, caplog, fee,
    ...
    trade = Trade.query.first()
    caplog.clear()
    kommunitas.create_stoploss_order(trade, 200)
    assert trade.stoploss_order_id is None
...
def test_create_stoploss_order_insufficient_funds(mocker, default_conf, caplog, fee,
    ...
    mock_insuf = mocker.patch('kommunitas.kommunitasbot.kommunitasBot.handle_insufficient_funds')
    mocker.patch.multiple(
        'kommunitas.exchange.Exchange',
        fetch_ticker=MagicMock(return_value={
            'bid': 0.00001172,
...
def test_handle_stoploss_on_exchange_trailing(mocker, default_conf, fee,
    ...
    # setting stoploss
    kommunitas.strategy.stoploss = -0.05

    # setting stoploss_on_exchange_interval to 60 seconds
    kommunitas.strategy.order_types['stoploss_on_exchange_interval'] = 60
    ...
    mocker.patch('kommunitas.exchange.bitmart.fetch_stoploss_order', stoploss_order_hanging)
    ...
    # price jumped 2x
    mocker.patch('kommunitas.exchange.Exchange.fetch_ticker', MagicMock(return_value={
        'bid': 0.00002344,
        'ask': 0.00002346,
        'last': 0.00002344
    }))
    ...
    mocker.patch('kommunitas.exchange.bitmart.cancel_stoploss_order', cancel_order_mock)
    mocker.patch('kommunitas.exchange.bitmart.stoploss', stoploss_order_mock)
    ...
    kommunitas.strategy.order_types['stoploss_on_exchange_interval'] = 0
    ...
    stoploss_order_mock.assert_called_once_with(amount=85.32423208,
                                                pair='ETH/BTC',
                                                order_types=kommunitas.strategy.order_types,
...
def test_handle_stoploss_on_exchange_trailing_error(mocker, default_conf, fee, caplog,
    ...
    mocker.patch('kommunitas.exchange.bitmart.cancel_stoploss_order',
                 side_effect=InvalidOrderException())
    mocker.patch('kommunitas.exchange.bitmart.fetch_stoploss_order',
                 return_value=stoploss_order_hanging)
    kommunitas.handle_trailing_stoploss_on_exchange(trade, stoploss_order_hanging)
    ...
    caplog.clear()
    cancel_mock = mocker.patch("kommunitas.exchange.bitmart.cancel_stoploss_order", MagicMock())
    mocker.patch("kommunitas.exchange.bitmart.stoploss", side_effect=ExchangeError())
    kommunitas.handle_trailing_stoploss_on_exchange(trade, stoploss_order_hanging)
...
def test_handle_stoploss_on_exchange_custom_stop(mocker, default_conf, fee,
    ...
    # setting stoploss
    kommunitas.strategy.custom_stoploss = lambda *args, **kwargs: -0.04
...
def test_tsl_on_exchange_compatible_with_edge(mocker, edge_conf, fee, caplog,
    ...
    edge_conf['max_open_trades'] = float('inf')
    edge_conf['dry_run_wallet'] = 999.9
    edge_conf['exchange']['name'] = 'bitmart'
    mocker.patch.multiple(
        'kommunitas.exchange.Exchange',
        fetch_ticker=MagicMock(return_value={
            'bid': 0.00001172,
    ...
    # setting stoploss
    kommunitas.strategy.stoploss = -0.02
    ...
    kommunitas.active_pair_whitelist = kommunitas.edge.adjust(kommunitas.active_pair_whitelist)
    ...
    mocker.patch('kommunitas.exchange.Exchange.fetch_stoploss_order', stoploss_order_hanging)
    ...
    mocker.patch('kommunitas.exchange.Exchange.cancel_stoploss_order', cancel_order_mock)
    mocker.patch('kommunitas.exchange.bitmart.stoploss', stoploss_order_mock)
...
def test_enter_positions(mocker, default_conf, caplog) -> None:
    ...
    mock_ct = mocker.patch('kommunitas.kommunitasbot.kommunitasBot.create_trade',
...
def test_enter_positions_exception(mocker, default_conf, caplog) -> None:
    ...
    mock_ct = mocker.patch(
        'kommunitas.kommunitasbot.kommunitasBot.create_trade',
...
def test_exit_positions(mocker, default_conf, limit_buy_order, caplog) -> None:
    ...
    mocker.patch('kommunitas.kommunitasbot.kommunitasBot.get_real_amount', return_value=90.81)
...
def test_exit_positions_exception(mocker, default_conf, limit_buy_order, caplog) -> None:
    ...
    mocker.patch(
        'kommunitas.kommunitasbot.kommunitasBot.handle_trade',
...
def test_update_trade_state_withorderdict(default_conf, trades_for_order, limit_buy_order, fee,
                                          mocker):
    mocker.patch('kommunitas.exchange.Exchange.get_trades_for_order', return_value=trades_for_order)
    # fetch_order should not be called!!
    mocker.patch('kommunitas.exchange.Exchange.fetch_order', MagicMock(side_effect=ValueError))
    patch_exchange(mocker)
    ...
    kommunitas.update_trade_state(trade, '123456', limit_buy_order)
...
def test_update_trade_state_exception(mocker, default_conf,
    ...
    mocker.patch(
        'kommunitas.kommunitasbot.kommunitasBot.get_real_amount',
        side_effect=DependencyException()
    )
    kommunitas.update_trade_state(trade, trade.open_order_id)
...
def test_update_trade_state_orderexception(mocker, default_conf, caplog) -> None:
    ...
    # Test raise of OperationalException exception
    grm_mock = mocker.patch("kommunitas.kommunitasbot.kommunitasBot.get_real_amount", MagicMock())
    kommunitas.update_trade_state(trade, trade.open_order_id)
...
def test_update_trade_state_sell(default_conf, trades_for_order, limit_sell_order_open,
    ...
    mocker.patch('kommunitas.wallets.Wallets.update', wallet_mock)
    ...
    kommunitas.update_trade_state(trade, trade.open_order_id, limit_sell_order)
...
def test_handle_trade(default_conf, limit_buy_order, limit_sell_order_open, limit_sell_order,
    ...
    patch_get_signal(kommunitas, value=(False, True, None))
    assert kommunitas.handle_trade(trade) is True
...
def test_handle_overlapping_signals(default_conf, ticker, limit_buy_order_open,
    ...
    kommunitas = kommunitasBot(default_conf)
    patch_get_signal(kommunitas, value=(True, True, None))
    kommunitas.strategy.min_roi_reached = MagicMock(return_value=False)
    ...
    patch_get_signal(kommunitas, value=(True, False, None))
    kommunitas.enter_positions()
    trades = Trade.query.all()
    nb_trades = len(trades)
    assert nb_trades == 1
    ...
    patch_get_signal(kommunitas, value=(False, False, None))
    assert kommunitas.handle_trade(trades[0]) is False
    trades = Trade.query.all()
    nb_trades = len(trades)
    assert nb_trades == 1
    assert trades[0].is_open is True
    ...
    patch_get_signal(kommunitas, value=(False, True, None))
    trades = Trade.query.all()
    assert kommunitas.handle_trade(trades[0]) is True
...
def test_handle_trade_roi(default_conf, ticker, limit_buy_order_open,
    ...
    kommunitas = get_patched_kommunitasbot(mocker, default_conf)
    patch_get_signal(kommunitas, value=(True, False, None))
    kommunitas.strategy.min_roi_reached = MagicMock(return_value=True)
    ...
    patch_get_signal(kommunitas, value=(False, True, None))
    assert kommunitas.handle_trade(trade)
...
def test_handle_trade_use_sell_signal(default_conf, ticker, limit_buy_order_open,
    ...
    patch_get_signal(kommunitas, value=(False, False, None))
    assert not kommunitas.handle_trade(trade)
...
def test_close_trade(default_conf, ticker, limit_buy_order, limit_buy_order_open, limit_sell_order,
    ...
    with pytest.raises(DependencyException, match=r'.*closed trade.*'):
        kommunitas.handle_trade(trade)


def test_bot_loop_start_called_once(mocker, default_conf, caplog):
    ftbot = get_patched_kommunitasbot(mocker, default_conf)
    mocker.patch('kommunitas.kommunitasbot.kommunitasBot.create_trade')
...
def test_check_handle_timedout_buy_usercustom(default_conf, ticker, limit_buy_order_old, open_trade,
    ...
    kommunitas.check_handle_timedout()
    assert cancel_order_mock.call_count == 0

    # Return false - trade remains open
    kommunitas.strategy.check_buy_timeout = MagicMock(return_value=False)
    kommunitas.check_handle_timedout()
    assert cancel_order_mock.call_count == 0
    trades = Trade.query.filter(Trade.open_order_id.is_(open_trade.open_order_id)).all()
    nb_trades = len(trades)
    assert nb_trades == 1
    assert kommunitas.strategy.check_buy_timeout.call_count == 1

    # Raise Keyerror ... (no impact on trade)
    kommunitas.strategy.check_buy_timeout = MagicMock(side_effect=KeyError)
    kommunitas.check_handle_timedout()
    assert cancel_order_mock.call_count == 0
    trades = Trade.query.filter(Trade.open_order_id.is_(open_trade.open_order_id)).all()
    nb_trades = len(trades)
    ...
    kommunitas.strategy.check_buy_timeout = MagicMock(return_value=True)
...
def test_check_handle_timedout_buy(default_conf, ticker, limit_buy_order_old, open_trade,
    ...
    assert kommunitas.strategy.check_buy_timeout.call_count == 0
...
def test_check_handle_timedout_sell_usercustom(default_conf, ticker, limit_sell_order_old, mocker,
    ...
    kommunitas.strategy.check_sell_timeout = MagicMock(return_value=False)
    # Return false - No impact
    kommunitas.check_handle_timedout()
    assert cancel_order_mock.call_count == 0
    assert rpc_mock.call_count == 0
    assert open_trade.is_open is False
    assert kommunitas.strategy.check_sell_timeout.call_count == 1

    kommunitas.strategy.check_sell_timeout = MagicMock(side_effect=KeyError)
    ...
    # Return True - sells!
    kommunitas.strategy.check_sell_timeout = MagicMock(return_value=True)
    kommunitas.check_handle_timedout()
    assert cancel_order_mock.call_count == 1
    assert rpc_mock.call_count == 1
...
def test_check_handle_timedout_sell(default_conf, ticker, limit_sell_order_old, mocker,
    ...
    assert kommunitas.strategy.check_sell_timeout.call_count == 0
...
def test_check_handle_timedout_partial_fee(default_conf, ticker, open_trade, caplog, fee,
    ...
    mocker.patch('kommunitas.wallets.Wallets.get_free', MagicMock(return_value=0))
    patch_exchange(mocker)
    mocker.patch.multiple(
        'kommunitas.exchange.Exchange',
        fetch_ticker=ticker,
...
def test_handle_cancel_buy(mocker, caplog, default_conf, limit_buy_order) -> None:
    ...
    mocker.patch('kommunitas.exchange.Exchange.cancel_order_with_result', cancel_order_mock)

    kommunitas = kommunitasBot(default_conf)
    kommunitas._notify_buy_cancel = MagicMock()
    ...
    assert kommunitas.handle_cancel_buy(trade, limit_buy_order, reason)
    assert cancel_order_mock.call_count == 1

    cancel_order_mock.reset_mock()
    caplog.clear()
    limit_buy_order['filled'] = 0.01
    assert not kommunitas.handle_cancel_buy(trade, limit_buy_order, reason)
    assert cancel_order_mock.call_count == 0
...
@pytest.mark.parametrize("limit_buy_order_canceled_empty", ['bitmart', 'ftx', 'kraken', 'bittrex'],
...
def test_handle_cancel_buy_exchanges(mocker, caplog, default_conf,
    ...
    cancel_order_mock = mocker.patch(
        'kommunitas.exchange.Exchange.cancel_order_with_result',
        return_value=limit_buy_order_canceled_empty)
    nofiy_mock = mocker.patch('kommunitas.kommunitasbot.kommunitasBot._notify_buy_cancel')
    kommunitas = kommunitasBot(default_conf)

    reason = CANCEL_REASON['TIMEOUT']
    trade = MagicMock()
    trade.pair = 'LTC/ETH'
    assert kommunitas.handle_cancel_buy(trade, limit_buy_order_canceled_empty, reason)
    assert cancel_order_mock.call_count == 0
...
def test_handle_cancel_sell_limit(mocker, default_conf, fee) -> None:
    ...
    mocker.patch('kommunitas.exchange.Exchange.get_rate', return_value=0.245441)
    ...
    order = {'remaining': 1,
             ...
             'status': "open"}
    reason = CANCEL_REASON['TIMEOUT']
    assert kommunitas.handle_cancel_sell(trade, order, reason)
    assert cancel_order_mock.call_count == 1
...
def test_handle_cancel_sell_cancel_exception(mocker, default_conf) -> None:
    ...
    patch_exchange(mocker)
    mocker.patch(
        'kommunitas.exchange.Exchange.cancel_order_with_result', side_effect=InvalidOrderException())
    ...
    order = {'remaining': 1,
             'amount': 1,
             'status': "open"}
    assert kommunitas.handle_cancel_sell(trade, order, reason) == 'error cancelling order'
...
def test_execute_sell_up(default_conf, ticker, fee, ticker_sell_up, mocker) -> None:
    ...
    kommunitas = kommunitasBot(default_conf)
    patch_get_signal(kommunitas)
    kommunitas.strategy.confirm_trade_exit = MagicMock(return_value=False)
    ...
    # Prevented sell ...
    kommunitas.execute_sell(trade=trade, limit=ticker_sell_up()['bid'],
...
def test_execute_sell_down(default_conf, ticker, fee, ticker_sell_down, mocker) -> None:
    ...
    kommunitas.execute_sell(trade=trade, limit=ticker_sell_down()['bid'],
...
def test_execute_sell_sloe_cancel_exception(mocker, default_conf, ticker, fee, caplog) -> None:
    kommunitas = get_patched_kommunitasbot(mocker, default_conf)
    mocker.patch('kommunitas.exchange.Exchange.cancel_stoploss_order',
                 side_effect=InvalidOrderException())
    mocker.patch('kommunitas.wallets.Wallets.get_free', MagicMock(return_value=300))
    create_order_mock = MagicMock(side_effect=[
        {'id': '12345554'},
        {'id': '12345555'},
    ])
    ...
    kommunitas.config['dry_run'] = False
    trade.stoploss_order_id = "abcd"

    kommunitas.execute_sell(trade=trade, limit=1234,
                           sell_reason=SellCheckTuple(sell_type=SellType.STOP_LOSS))
    assert create_order_mock.call_count == 2
...
def test_may_execute_sell_after_stoploss_on_exchange_hit(default_conf, ticker, fee,
    ...
    mocker.patch('kommunitas.exchange.Exchange.fetch_stoploss_order', stoploss_executed)
...
def test_execute_sell_market_order(default_conf, ticker, fee,
    ...
    kommunitas.config['order_types']['sell'] = 'market'
...
def test_execute_sell_insufficient_funds_error(default_conf, ticker, fee,
    ...
    sell_reason = SellCheckTuple(sell_type=SellType.ROI)
    assert not kommunitas.execute_sell(trade=trade, limit=ticker_sell_up()['bid'],
...
def test_sell_profit_only_enable_profit(default_conf, limit_buy_order, limit_buy_order_open,
    ...
    kommunitas.strategy.sell_profit_offset = 0.0
    assert kommunitas.handle_trade(trade) is True
...
def test_sell_profit_only_enable_loss(default_conf, limit_buy_order, limit_buy_order_open,
    ...
    kommunitas = kommunitasBot(default_conf)
    patch_get_signal(kommunitas)
    kommunitas.strategy.stop_loss_reached = MagicMock(return_value=SellCheckTuple(
...
def test_sell_not_enough_balance(default_conf, limit_buy_order, limit_buy_order_open,
    ...
    trade.update(limit_buy_order)
    patch_get_signal(kommunitas, value=(False, True, None))
    mocker.patch('kommunitas.wallets.Wallets.get_free', MagicMock(return_value=trade.amount * 0.985))
...
def test__safe_sell_amount(default_conf, fee, caplog, mocker):
    ...
    amount = 95.33
    amount_wallet = 95.29
    mocker.patch('kommunitas.wallets.Wallets.get_free', MagicMock(return_value=amount_wallet))
    wallet_update = mocker.patch('kommunitas.wallets.Wallets.update')
    trade = Trade(
        pair='LTC/ETH',
        amount=amount,
        exchange='bitmart',
    ...
    assert kommunitas._safe_sell_amount(trade.pair, trade.amount) == amount_wallet
    assert log_has_re(r'.*Falling back to wallet-amount.', caplog)
    assert wallet_update.call_count == 1
    caplog.clear()
    wallet_update.reset_mock()
    assert kommunitas._safe_sell_amount(trade.pair, amount_wallet) == amount_wallet
...
def test__safe_sell_amount_error(default_conf, fee, caplog, mocker):
    ...
    with pytest.raises(DependencyException, match=r"Not enough amount to sell."):
        assert kommunitas._safe_sell_amount(trade.pair, trade.amount)
...
def test_locked_pairs(default_conf, ticker, fee, ticker_sell_down, mocker, caplog) -> None:
    ...
    assert kommunitas.strategy.is_pair_locked(trade.pair)
...
def test_trailing_stop_loss(default_conf, limit_buy_order_open, limit_buy_order,
    ...
    mocker.patch('kommunitas.exchange.Exchange.fetch_ticker',
...
def test_get_real_amount_quote(default_conf, trades_for_order, buy_order_fee, fee, caplog, mocker):
    ...
    assert kommunitas.get_real_amount(trade, buy_order_fee) == amount - (amount * 0.001)
...
def test_get_real_amount_quote_dust(default_conf, trades_for_order, buy_order_fee, fee,
                                    caplog, mocker):
    mocker.patch('kommunitas.exchange.Exchange.get_trades_for_order', return_value=trades_for_order)
    walletmock = mocker.patch('kommunitas.wallets.Wallets.update')
    mocker.patch('kommunitas.wallets.Wallets.get_free', return_value=8.1122)
    amount = sum(x['amount'] for x in trades_for_order)
    trade = Trade(
        pair='LTC/ETH',
        amount=amount,
    ...
    assert kommunitas.get_real_amount(trade, buy_order_fee) == amount
...
def test_get_real_amount_no_currency_in_fee(default_conf, trades_for_order, buy_order_fee,
    ...
    # Amount does not change
    assert kommunitas.get_real_amount(trade, limit_buy_order) == amount
...
def test_get_real_amount_multi(default_conf, trades_for_order2, buy_order_fee, caplog, fee, mocker):
    mocker.patch('kommunitas.exchange.Exchange.get_trades_for_order', return_value=trades_for_order2)
...
def test_get_real_amount_multi2(default_conf, trades_for_order3, buy_order_fee, caplog, fee,
    ...
    mocker.patch('kommunitas.exchange.Exchange.get_trades_for_order', return_value=trades_for_order3)
    ...
    # Amount is reduced by "fee"
    assert kommunitas.get_real_amount(trade, buy_order_fee) == amount - (amount * 0.0005)
    assert log_has('Applying fee on amount for Trade(id=None, pair=LTC/ETH, amount=8.00000000, '
...
def test_get_real_amount_fromorder(default_conf, trades_for_order, buy_order_fee, fee,
    ...
    mocker.patch('kommunitas.exchange.Exchange.get_trades_for_order',
    ...
    mocker.patch('kommunitas.exchange.Exchange.fetch_ticker', side_effect=ExchangeError)

    # Amount is reduced by "fee"
    assert kommunitas.get_real_amount(trade, limit_buy_order) == amount - 0.004
    assert log_has('Applying fee on amount for Trade(id=None, pair=LTC/ETH, amount=8.00000000, '
                   'open_rate=0.24544100, open_since=closed) (from 8.0 to 7.996).',
                   caplog)
...
def test_get_real_amount_wrong_amount(default_conf, trades_for_order, buy_order_fee, fee, mocker):
    ...
    with pytest.raises(DependencyException, match=r"Half bought\? Amounts don't match"):
        kommunitas.get_real_amount(trade, limit_buy_order)
...
def test_get_real_amount_wrong_amount_rounding(default_conf, trades_for_order, buy_order_fee, fee,
    ...
    assert isclose(kommunitas.get_real_amount(trade, limit_buy_order), amount - (amount * 0.001),
...
def test_get_real_amount_open_trade(default_conf, fee, mocker):
    ...
    kommunitas = get_patched_kommunitasbot(mocker, default_conf)
    assert kommunitas.get_real_amount(trade, order) == amount
...
def test_apply_fee_conditional(default_conf, fee, caplog, mocker,
    ...
    walletmock = mocker.patch('kommunitas.wallets.Wallets.update')
    mocker.patch('kommunitas.wallets.Wallets.get_free', return_value=wallet)
    trade = Trade(
        pair='LTC/ETH',
        amount=amount,
        exchange='bitmart',
    ...
    walletmock.reset_mock()
    # Amount is kept as is
    assert kommunitas.apply_fee_conditional(trade, 'LTC', amount, fee_abs) == amount_exp
    assert walletmock.call_count == 1
...
def test_order_book_depth_of_market(default_conf, ticker, limit_buy_order_open, limit_buy_order,
    ...
    patch_RPCManager(mocker)
    patch_exchange(mocker)
    mocker.patch('kommunitas.exchange.Exchange.fetch_l2_order_book', order_book_l2)
    mocker.patch.multiple(
        'kommunitas.exchange.Exchange',
        fetch_ticker=ticker,
        create_order=MagicMock(return_value=limit_buy_order_open),
...
def test_order_book_bid_strategy1(mocker, default_conf, order_book_l2) -> None:
    ...
    kommunitas = kommunitasBot(default_conf)
    assert kommunitas.exchange.get_rate('ETH/BTC', refresh=True, side="buy") == 0.043935
...
def test_order_book_bid_strategy_exception(mocker, default_conf, caplog) -> None:
    ...
    with pytest.raises(PricingError):
        kommunitas.exchange.get_rate('ETH/BTC', refresh=True, side="buy")
...
def test_check_depth_of_market_buy(default_conf, mocker, order_book_l2) -> None:
    ...
    assert kommunitas._check_depth_of_market_buy('ETH/BTC', conf) is False
...
def test_order_book_ask_strategy(default_conf, limit_buy_order_open, limit_buy_order, fee,
    ...
    mocker.patch('kommunitas.exchange.Exchange.fetch_l2_order_book',
...
def test_startup_state(default_conf, mocker):
    ...
    mocker.patch('kommunitas.exchange.Exchange.exchange_has', MagicMock(return_value=True))
    worker = get_patched_worker(mocker, default_conf)
    assert worker.kommunitas.state is State.RUNNING
...
def test_startup_trade_reinit(default_conf, edge_conf, mocker):
    ...
    mocker.patch('kommunitas.persistence.Trade.stoploss_reinitialization', reinit_mock)
    ...
    ftbot = get_patched_kommunitasbot(mocker, edge_conf)
    ftbot.startup()
...
def test_sync_wallet_dry_run(mocker, default_conf, ticker, fee, limit_buy_order_open, caplog):
    ...
    bot = get_patched_kommunitasbot(mocker, default_conf)
...
def test_cancel_all_open_orders(mocker, default_conf, fee, limit_buy_order, limit_sell_order):
    ...
    buy_mock = mocker.patch('kommunitas.kommunitasbot.kommunitasBot.handle_cancel_buy')
    sell_mock = mocker.patch('kommunitas.kommunitasbot.kommunitasBot.handle_cancel_sell')

    kommunitas = get_patched_kommunitasbot(mocker, default_conf)
    create_mock_trades(fee)
    trades = Trade.query.all()
    assert len(trades) == MOCK_TRADE_COUNT
    kommunitas.cancel_all_open_orders()
...
def test_check_for_open_trades(mocker, default_conf, fee):
    ...
    kommunitas.check_for_open_trades()
    assert kommunitas.rpc.send_msg.call_count == 0
    ...
    kommunitas.check_for_open_trades()
    assert kommunitas.rpc.send_msg.call_count == 1
    assert 'Handle these trades manually' in kommunitas.rpc.send_msg.call_args[0][0]['status']
...
def test_update_open_orders(mocker, default_conf, fee, caplog):
    ...
    mocker.patch('kommunitas.exchange.Exchange.fetch_order', return_value=matching_buy_order)
    kommunitas.update_open_orders()
...
def test_update_closed_trades_without_assigned_fees(mocker, default_conf, fee):
    ...
    mocker.patch('kommunitas.exchange.Exchange.fetch_order_or_stoploss_order',
                 side_effect=[
    ...
    kommunitas.update_closed_trades_without_assigned_fees()
...
def test_reupdate_buy_order_fees(mocker, default_conf, fee, caplog):
    kommunitas = get_patched_kommunitasbot(mocker, default_conf)
    mock_uts = mocker.patch('kommunitas.kommunitasbot.kommunitasBot.update_trade_state')

    create_mock_trades(fee)
    trades = Trade.get_trades().all()

    kommunitas.reupdate_buy_order_fees(trades[0])
    ...
    kommunitas.reupdate_buy_order_fees(trade)
    assert log_has_re(r"Trying to reupdate buy fees for .*", caplog)
...
def test_handle_insufficient_funds(mocker, default_conf, fee):
    kommunitas = get_patched_kommunitasbot(mocker, default_conf)
    mock_rlo = mocker.patch('kommunitas.kommunitasbot.kommunitasBot.refind_lost_order')
    mock_bof = mocker.patch('kommunitas.kommunitasbot.kommunitasBot.reupdate_buy_order_fees')
    create_mock_trades(fee)
    trades = Trade.get_trades().all()

    # Trade 0 has only a open buy order, no closed order
    kommunitas.handle_insufficient_funds(trades[0])
    ...
    kommunitas.handle_insufficient_funds(trades[1])
    ...
    kommunitas.handle_insufficient_funds(trades[2])
    assert mock_rlo.call_count == 1
    assert mock_bof.call_count == 0
    ...
    kommunitas.handle_insufficient_funds(trades[3])
    assert mock_rlo.call_count == 0
    assert mock_bof.call_count == 1
...
def test_refind_lost_order(mocker, default_conf, fee, caplog):
    ...
    mock_fo = mocker.patch('kommunitas.exchange.Exchange.fetch_order_or_stoploss_order',
    ...
    kommunitas.refind_lost_order(trade)
    order = mock_order_1()
    ...
    kommunitas.refind_lost_order(trades[4])
