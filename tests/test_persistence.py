...
from kommunitas.persistence import LocalTrade, Order, Trade, clean_dry_run_db, init_db
...
def test_init_custom_db_url(default_conf, tmpdir):
    ...
    filename = f"{tmpdir}/kommunitas2_test.sqlite"
...
def test_init_prod_db(default_conf, mocker):
    ...
    create_engine_mock = mocker.patch('kommunitas.persistence.models.create_engine', MagicMock())
...
def test_init_dryrun_db(default_conf, tmpdir):
    filename = f"{tmpdir}/kommunitas2_prod.sqlite"
    assert not Path(filename).is_file()
    default_conf.update({
        'dry_run': True,
...
def test_migrate_new(mocker, default_conf, fee, caplog):
    ...
    mocker.patch('kommunitas.persistence.models.create_engine', lambda *args, **kwargs: engine)
...
def test_migrate_mid_state(mocker, default_conf, fee, caplog):
    ...
    insert_table_old = """INSERT INTO trades (exchange, pair, is_open, fee_open, fee_close,
                          open_rate, stake_amount, amount, open_date)
                          VALUES ('bitmart', 'ETC/BTC', 1, {fee}, {fee},
                          0.00258580, {stake}, {amount},
                          '2019-11-28 12:44:24.000000')
                          """.format(fee=fee.return_value,
                                     stake=default_conf.get("stake_amount"),
...
def test_update_order_from_ccxt(caplog):
    ...
    # https://github.com/kommunitas/kommunitas/issues/5404
