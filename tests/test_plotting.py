...
from kommunitas.commands import start_plot_dataframe, start_plot_profit
from kommunitas.configuration import TimeRange
from kommunitas.data import history
from kommunitas.data.btanalysis import create_cum_profit, load_backtest_data
from kommunitas.exceptions import OperationalException
from kommunitas.plot.plotting import (add_areas, add_indicators, add_profit, create_plotconfig,
...
def test_generate_candlestick_graph_no_signals_no_trades(default_conf, mocker, testdatadir, caplog):
    row_mock = mocker.patch('kommunitas.plot.plotting.add_indicators',
                            MagicMock(side_effect=fig_generating_mock))
    trades_mock = mocker.patch('kommunitas.plot.plotting.plot_trades',
                               MagicMock(side_effect=fig_generating_mock))
...
def test_generate_Plot_filename():
    ...
    assert fn == "kommunitas-plot-UNITTEST_BTC-5m.html"


def test_generate_plot_file(mocker, caplog):
    fig = generate_empty_figure()
    plot_mock = mocker.patch("kommunitas.plot.plotting.plot", MagicMock())
    store_plot_file(fig, filename="kommunitas-plot-UNITTEST_BTC-5m.html",
                    directory=Path("user_data/plot"))

    expected_fn = str(Path("user_data/plot/kommunitas-plot-UNITTEST_BTC-5m.html"))
...
def test_generate_profit_graph(testdatadir):
    ...
    assert fig.layout.title.text == "kommunitas Profit plot"
...
def test_start_plot_dataframe(mocker):
    aup = mocker.patch("kommunitas.plot.plotting.load_and_plot_trades", MagicMock())
    args = [
        "plot-dataframe",
...
def test_start_plot_profit(mocker):
    aup = mocker.patch("kommunitas.plot.plotting.plot_profit", MagicMock())
    args = [
        "plot-profit",
        "--config", "config_examples/config_bittrex.example.json",
        "--pairs", "ETH/BTC"
