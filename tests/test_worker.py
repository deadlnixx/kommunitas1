...
def test_worker_state(mocker, default_conf, markets) -> None:
    ...
    assert worker.kommunitas.state is State.STOPPED


def test_worker_running(mocker, default_conf, caplog) -> None:
    mock_throttle = MagicMock()
    mocker.patch('kommunitas.worker.Worker._throttle', mock_throttle)
    mocker.patch('kommunitas.persistence.Trade.stoploss_reinitialization', MagicMock())
    ...
    assert worker.kommunitas.strategy
    assert worker.kommunitas.strategy.dp
    assert isinstance(worker.kommunitas.strategy.dp, DataProvider)
...
def test_worker_stopped(mocker, default_conf, caplog) -> None:
    ...
    worker = get_patched_worker(mocker, default_conf)
    worker.kommunitas.state = State.STOPPED
...
def test_worker_heartbeat_running(default_conf, mocker, caplog):
    ...
    worker.kommunitas.state = State.RUNNING
